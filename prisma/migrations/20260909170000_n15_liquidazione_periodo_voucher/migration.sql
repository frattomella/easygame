-- ------------------------------------------------------------------------
-- N15 — La liquidazione di un periodo, e il movimento bancario che ne esce
-- ------------------------------------------------------------------------
--
-- Il movimento bancario di una liquidazione **esiste gia**: la vista
-- `accounting_ledger_lines` proietta ogni riga di `funding_settlements` come
-- una riga del registro, con il conto, il verso, la causale e la sede. Non si
-- materializza niente in `accounting_entries` — `OWNERSHIP.md` lo vieta per
-- nome, e `WRITABLE_SOURCE_DOMAINS` non contiene `FUNDING_SETTLEMENT`.
--
-- Questa migrazione aggiunge le tre cose che mancavano perche quel movimento
-- fosse **leggibile** e la scrittura **ripetibile**, e non una riga di piu.
--
-- 1. `idempotency_key` — il doppio invio non deve produrre due bonifici.
--    L'indice unico parziale e la difesa vera: una lettura prima della
--    scrittura e una lettura vecchia, e due richieste simultanee la
--    troverebbero tutte e due libera. Qui la seconda sbatte sull'archivio.
--
-- 2. `beneficiary_athlete_id` — **quando il beneficiario e uno solo.**
--    Un ente liquida in blocco: un bonifico per venti atleti e tre mesi, ed e
--    la ragione per cui `funding_settlement_lines` esiste. Ma la liquidazione
--    di un singolo periodo — il gesto che N15 introduce — ne ha esattamente
--    uno, e chi legge l'estratto conto deve poterlo vedere senza percorrere
--    tre join. Nullo quando i beneficiari sono zero o piu d'uno: un campo che
--    mente e peggio di un campo che tace. Lo calcola il dominio dentro la
--    transazione, dalle righe: non si riceve dal chiamante e non puo divergere.
--
-- 3. `description_snapshot` — la descrizione **congelata** al momento della
--    scrittura, come l'etichetta della causale e l'ambito. Serve a scrivere
--    «Incasso voucher Sport e Salute — Mario Rossi — ottobre 2026» invece di
--    «Liquidazione - Sport e Salute», e a scriverlo **una volta sola**: la
--    vista e il suo gemello TypeScript la leggono e basta, quindi non c'e una
--    seconda regola di composizione che possa divergere fra SQL e TypeScript.
--    Congelarla e la stessa disciplina di `operation_type_label_snapshot`: un
--    atleta che cambia cognome non riscrive un estratto conto gia stampato.
--
-- La vista si ricrea con `CREATE OR REPLACE`, che conserva nome, colonne, tipi
-- e ordine: cambiano le espressioni di **due** colonne su **un** ramo — la
-- descrizione e il testo di ricerca del ramo delle liquidazioni.
-- ------------------------------------------------------------------------

ALTER TABLE "funding_settlements"
  ADD COLUMN IF NOT EXISTS "idempotency_key" TEXT,
  ADD COLUMN IF NOT EXISTS "beneficiary_athlete_id" UUID,
  ADD COLUMN IF NOT EXISTS "description_snapshot" TEXT;

-- Parziale: le liquidazioni gia registrate non hanno chiave, e due `NULL` non
-- si somigliano. Per club, perche la chiave la genera il browser di un club.
CREATE UNIQUE INDEX IF NOT EXISTS "funding_settlements_idempotency_key_key"
  ON "funding_settlements" ("organization_id", "idempotency_key")
  WHERE "idempotency_key" IS NOT NULL;

CREATE INDEX IF NOT EXISTS "funding_settlements_beneficiary_athlete_id_idx"
  ON "funding_settlements" ("beneficiary_athlete_id");

-- `SET NULL` e non `RESTRICT`: il beneficiario e una **comodita di lettura**,
-- non la fonte del legame. Quella resta `funding_settlement_lines`, che ha la
-- sua chiave esterna `RESTRICT` verso il maturato. Cancellare un atleta non
-- deve poter bloccare la storia di un bonifico, ne cancellarla.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'funding_settlements_beneficiary_athlete_id_fkey'
  ) THEN
    ALTER TABLE "funding_settlements"
      ADD CONSTRAINT "funding_settlements_beneficiary_athlete_id_fkey"
      FOREIGN KEY ("beneficiary_athlete_id") REFERENCES "athletes"("id")

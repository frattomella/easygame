...
/*
  L'import da file non scrive piu da qui: passa da `POST /api/v1/athletes/import`
  (ADR-0195, `src/lib/server/athlete-import.ts`), una transazione per atleta
  sul server, idempotente per lotto. Il lettore a scaglioni che viveva qui —
  un POST con l'elenco e un PUT di appartenenze per scheda — e stato tolto:
  creava le categorie del file dal client e non sapeva riprovare senza
  doppioni.
*/

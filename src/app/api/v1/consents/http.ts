...
import type { ConsentAccessScope } from "@/lib/server/consents";

/**
 * Le tre righe che ogni rotta dei consensi ripeterebbe.
 *
 ...
 * lo ricava dalle membership: e cio che permette al servizio di applicare la
 * matrice del §13 senza che nessuna rotta debba ricordarsi di verificarla.
 */
...
export type ResolvedConsentScope =
  | {
      scope: ConsentAccessScope;
      actorEmail: string | null;
      response?: undefined;
    }
  | { scope?: undefined; actorEmail?: undefined; response: NextResponse };
 ...
 */
export const resolveConsentScope = async (
  request: Request,
  requestedOrganizationId?: string | null,
): Promise<ResolvedConsentScope> => {
  ...
  return { scope, actorEmail: session.db.user?.email || null };
};

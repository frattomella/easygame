 ...
 *
 * Stessa forma di `src/app/api/v1/athlete-accounts/shared.ts` e di
 * `src/app/api/v1/trainer-accounts/[trainerId]/shared.ts`: la sessione e il
 * club attivo si risolvono qui, il permesso lo verifica il dominio
 * (`profile-account-links.ts`).
 */
...
export type ContestoGenitore = {
  params: { athleteId: string; guardianId: string };
};

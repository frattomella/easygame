...
| --- | --- | --- | --- | --- |
| V1 | 13 (assente, non disabilitato) | `src/components/modulistica/v2/template-editor-view.tsx:67` | `Select` «Di chi parla» con `disabled={!canManage}`; il resto della schermata e in sola lettura via `readOnly` dell'editor. Preferire un valore in lettura (`DetailCard`) quando `!canManage` | Basso — **chiuso** (`9f530ab`+1): il valore si legge come occhiello + testo quando `!canManage` |

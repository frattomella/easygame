...
| E-13 | Rimborsi e storni | `DONE` **il rimborso registrato**, `PARTIAL` **il rimborso avviato da EasyGame** | Un rimborso e un **movimento che conta**, distinto dallo storno: 130 incassati − 30 rimborsati = rata PARZIALE con residuo 30. La commissione restituita e proporzionale a quella trattenuta, non ricalcolata su oggi — e questo **e stato provato contro Stripe** il 2026-08-27, con il rimborso avviato dal cruscotto del provider. Dal 2026-08-27 il rimborso **si avvia anche da EasyGame**: pulsante nel dettaglio dell'incasso, importo parziale o totale, e il movimento resta scritto dall'evento firmato — finche non arriva, la riga dice «in elaborazione» ([ADR-0065](18-decision-log.md#adr-0065--il-rimborso-si-avvia-da-easygame-a-scriverlo-nel-registro-resta-levento-firmato)). **Quel percorso non ha ancora fatto un giro reale**: il trasporto verso Stripe e sostituito nei test, ed e esattamente la classe di prova che a questo blocco ha gia lasciato passare quattro difetti | 79 test: 27 sul dominio puro, 30 sul servizio, 22 sulle invarianti di interfaccia | No, **a condizione** che il giro reale sia eseguito prima di dichiarare chiuso Payment System V1 |
| E-14 | Billing EasyGame separato dagli incassi dei club | `PARTIAL` | [ADR-0051](18-decision-log.md#adr-0051--due-flussi-stripe-due-account-due-segreti-il-denaro-delle-famiglie-non-e-il-fatturato-di-easygame): due account, due endpoint webhook, due segreti. La **separazione e provata a runtime**: ogni segreto apre solo il proprio endpoint (400 incrociati sui due sensi) e un evento di incasso con firma Billing valida viene comunque ignorato, senza toccare il registro dell'atleta. Resta che **nessuna sottoscrizione reale** e mai stata creata | `platform-billing`, 14 test, piu la prova incrociata in sandbox | No |
...
| E-10 | Checkout online reale | `DONE` | **Giro reale eseguito** nella Sandbox Cedi Soft il 2026-08-27: connected account creato dal percorso EasyGame (Accounts v2, ADR-0061), onboarding completato, 50 € + 80 € su una rata da 130 €, application fee 1%, webhook, idempotenza, rimborso parziale e totale con commissione proporzionale, pagamento rifiutato. Il collaudo ha trovato **quattro difetti di contabilita**, tutti corretti e coperti da test (ADR-0062, ADR-0063) | 1.632 test; le prove a runtime sono nel registro incassi e nel cruscotto Stripe | Si |
| E-11 | Le tre decisioni Connect | `DONE` | Tutte e tre prese e **verificate sul campo**. **Percentuale**: 1% di riserva, override di club a 0,75% con decorrenza, non retroattivo sugli incassi gia congelati. **Tipo di account**: `dashboard: "full"`, l'equivalente Standard — il club vede il proprio cruscotto Stripe e i dati KYC li raccoglie Stripe, non EasyGame. **Saldi negativi**: `losses_collector: "stripe"`, e con essi `fees_collector: "stripe"`. Le due responsabilita **si congelano alla creazione** dell'account: non sono correggibili dopo | [ADR-0061](18-decision-log.md#adr-0061--un-account-connesso-nasce-sulla-accounts-v2-il-resto-dellintegrazione-resta-sulla-v1) | Si |

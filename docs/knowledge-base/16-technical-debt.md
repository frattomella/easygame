...
| SW-13 | **`CRON_SECRET` non e configurato su staging** | `vercel.json` registra il giro notturno alle 3:30, ma senza quella variabile la porta `GET /api/v1/sport-work/scheduler` risponde **503** e il giro non parte mai. Il comportamento del codice e corretto — chiuso per difetto, non aperto per comodita — la configurazione no: va impostata su staging e su produzione prima di considerare attiva l'agenda automatica |

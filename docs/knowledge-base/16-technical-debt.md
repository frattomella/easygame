...
| # | Cosa | Perche non e stato corretto qui |
|---|---|---|
| **W4-B1** | **La scheda «Previsti» non esiste piu, e con lei l'unica interfaccia di `expected_income` / `expected_expenses`.** I dati sono intatti e leggibili dal CRUD generico; quello che manca e la schermata. Non e stata riportata perche si reggeva sull'aggregatore nel browser che la Wave ha rimosso, e perche la sua finestra di creazione scriveva con `addClubData` — cioe leggeva la colonna JSON intera, faceva `append` e la risalvava dal browser: due segreterie nello stesso minuto e la seconda scrittura cancella la prima | Un previsto e un **impegno futuro**, non un fatto di cassa: non appartiene ad `accounting_entries` e riportarlo in prima nota rimetterebbe crediti e cassa nella stessa lista. Ha bisogno di un dominio proprio (una tabella e una rotta) o di essere dichiarato fuori dalla V1. E una decisione, non una riga di codice |
| **W4-B2** | **`listAccountingEntries` non somma: restituisce una pagina e il numero totale di righe, non entrate e uscite del periodo.** La prima nota mostra i totali per cassa **solo quando la pagina contiene tutte le righe del filtro**, e quando non li puo garantire lo dice invece di mostrarli. Su un anno con piu di cento movimenti i due riquadri restano quindi muti | Il totale del periodo e una **grandezza del server** e va calcolato dove i filtri sono gia applicati: significa cambiare `src/lib/server/accounting.ts`, che questa lane non possiede in questa tornata. Il posto giusto e `src/lib/accounting/reporting.ts` (W4-D), che possiede il rendiconto — sommarlo nel browser sarebbe il numero duplicato che il §28 del piano vieta |

...
| **D-AUD-32** | ~~**CHIUSO**~~ | (Reperto R1 dell'hostile audit del mandato) `createClubEventsBatch` non lasciava traccia quando l'intero blocco veniva scartato per campo chiuso o era interamente in conflitto: il registro non poteva distinguere "non ancora dovuto" da "girato, ha escluso tutto". Le due uscite anticipate scrivono ora la stessa riga di audit della creazione riuscita, con `generati: 0` — vedi [ADR-0179](18-decision-log.md#adr-0179--laudit-ostile-del-mandato-un-blocco-interamente-scartato-non-lasciava-traccia-r1). L'anteprima resta senza audit, per costruzione. `tests/server/audit-blocco-interamente-scartato.test.mjs` (3 prove) |
| **D-AUD-33** | Medium | **La riga di audit di `applyWeeklyScheduleSlotChanges` e un riepilogo, non un dettaglio.** `weekly_schedule.apply_impact` porta `slot`/`aggiornati`/`scartati` come conteggi sull'intera chiamata, non quali slot, quali eventi, quali motivi di scarto per riga — quel dettaglio esiste (`skippedReasons`) ma solo nel valore restituito a chi ha chiamato, non nel registro. Chi rivede l'audit un mese dopo vede "8 slot, 104 aggiornati, 0 scartati" e non puo risalire a quali fasce. Non verificato se due chiamate concorrenti sullo stesso club producano due righe correttamente distinte o si confondano nell'attribuzione — segnalato dall'hostile audit, non riconfermato in questo giro | Portare `skippedReasons` (o un estratto) nel `metadata` della riga di audit, non solo nel valore di ritorno; verificare l'attribuzione sotto due chiamate concorrenti con una sonda dedicata |
| **D-AUD-34** | Medium | **La rotta generica lascia un audit senza dettaglio quando scrive `settings.trainingAutomation`.** Confermato in `src/app/api/v1/[resource]/[id]/route.ts`: `auditResourceWrite` chiama `recordAuditEvent` senza `metadata` — solo risorsa e id, mai quale campo e cambiato. Chi ha il permesso e disattiva l'automazione, sposta `generateDaysAhead`, o modifica una sospensione dalla rotta generica lascia una riga indistinguibile da qualunque altra modifica al club. Le due rotte dedicate (generazione, impatto) hanno gia un `metadata` ricco: questa no | Un `metadata` che dica almeno quali chiavi di primo livello di `settings` sono cambiate, per la rotta generica — non solo per `trainingAutomation` |
| **D-AUD-35** | Medium | **Un nome di categoria ambiguo (zero o due o piu corrispondenze) fa tornare `resolveCategoryId` il valore grezzo, non `null`** (`category-utils.ts`). La risoluzione di WP-08 (`resolveSlotOccurrenceKey`) e quella della generazione vera usano la stessa funzione e quindi la stessa ambiguita — non sono per questo diverse fra loro — ma un club con due categorie omonime (stesso nome, sedi diverse, ADR-0155) puo vedere la chiave di deduplica collassare sul nome invece che sull'id per **entrambi** i percorsi, ed e un rischio piu profondo e preesistente della sola training-automation | Non affrontato in questo mandato per restare nello scope: il rimedio e probabilmente nel dominio dell'identita di categoria (`category-utils.ts`), non qui |
| **D-AUD-36** | Low | **L'anteprima e l'applicazione di WP-08 non condividono un `now`**: la rotta (`schedule-impact/route.ts`) non lo passa a nessuna delle due, ed entrambe ricadono su `new Date()` al proprio istante di chiamata. Fra "controlla l'impatto" e "conferma" passano in pratica secondi o minuti di digitazione umana: un evento il cui inizio cade esattamente sul confine passato/futuro in quella finestra puo contarsi nell'anteprima e non nell'applicazione, o viceversa — una finestra stretta, non una divergenza sistematica | Il client potrebbe portare avanti l'istante dell'anteprima e passarlo all'applicazione come `now`, se la finestra risultasse in pratica piu larga di quanto sembri qui |
| **D-AUD-37** | ~~**CHIUSO**~~ | **Un terzo lettore con la stessa ambiguita di D-AUD-35, sul lato display.** Segnalazione utente sul club pilota Fortitudo Scauri: la creazione/modifica di un allenamento mostrava tre voci per due squadre reali ("Pulcini", "Pulcini", "Pulcini · S. Cosma") invece di due ("Pulcini · Scauri", "Pulcini · S. Cosma"). Causa: due categorie *diverse* chiamate entrambe "Pulcini" (un identificativo per sede, dato storico precedente ai gruppi operativi) hanno ciascuna un solo gruppo attivo, e `TrainingGroupSelector.tsx`/`WeeklyTrainingSchedulePanel.tsx` decidevano se accostare la sede contando i gruppi per `categoryId` — con un solo gruppo per identificativo quel conto vale sempre uno, e la sede non compariva mai. Corretto a contare per `categoryName` normalizzato (non e un ripiego sul nome per l'*identita*: quella resta l'id, invariata; e solo la domanda "questo nome, sullo schermo, ne nomina piu di uno?"). Nello stesso pannello, il dialogo "Modifica Allenamento Programma" aveva un secondo difetto indipendente: la select "Gruppo" leggeva le sue `<option>` da `categories` (l'anagrafica canonica, non filtrata) con `value={category.id}`, mentre il valore legato e l'`onChange` interpretavano quel valore come un id di **gruppo** (`group:categoryId:siteId`) — i due non si incontravano mai, la selezione restava vuota e cambiarla non cambiava niente. Il dialogo "Aggiungi" dello stesso pannello non aveva questo secondo difetto. Sonda read-only su staging: 4 categorie canoniche (2 "Pulcini", 2 "Scoiattoli"), 6 righe `category_groups` (4 attive, 2 residui storici inattivi, mai promossi a opzione); nessun `club_events`/`weekly_schedule` con un `categoryId` corrotto (name-as-id o group-id-as-category-id). Nessun dato toccato. `tests/lib/categoria-omonima-programma-allenamenti.test.mjs`, `tests/ui/training-categoria-groupOptions.test.mjs` |
| **D-AUD-38** | Low | **Due categorie omonime senza nessun `category_groups` configurato restano indistinguibili anche dopo D-AUD-37**: la correzione disambigua per sede, ma un club che non ha mai configurato le sedi/gruppi (`category_groups: null`) non ha nessuna sede da accostare — il secondo club "Fortitudo Scauri" trovato dalla sonda (`ef5317db-8e49-4401-b452-b915db2376f0`, distinto dal club pilota `4139ddd1-...`) e "QA UAT Club" hanno categorie omonime ("Esordienti", "Pulcini 23", "Dr3", "Aquilotti", "Scoiattoli") con zero gruppi: restano scritte allo stesso modo ovunque. Segnalato, non affrontato: non e un difetto del lettore, e un'ambiguita del dato che nessuna etichetta puo risolvere senza altra informazione | Se il caso si rivelasse frequente: un avviso in `/categories` quando due categorie condividono nome e non hanno gruppi a distinguerle, cosi la segreteria lo sistema alla fonte (rinomina, o configura le sedi) invece di scoprirlo in una tendina |
| **D-AUD-39** | ~~**CHIUSO**~~ | **Un riferimento legacy nell'appartenenza di un atleta poteva diventare un terzo gruppo selezionabile.** Segnalazione di regressione dopo D-AUD-37: lo staging avrebbe continuato a mostrare tre voci ("Pulcini · S. Cosma" due volte). La sonda su `athlete_category_memberships` del club pilota ha trovato, per nove atleti, una **seconda** riga per la stessa appartenenza con `category_id` corrotto a un'etichetta (`"Pulcini - S. Cosma"`) — ma quella coppia **si autorepara** in `dedupeMemberships` (`athlete-category-memberships.ts`): la riga gemella con l'identificativo vero fa da catalogo implicito, e la riga corrotta si fonde su quella, senza creare niente di nuovo — verificato con una sonda dedicata, nessuna voce fantasma nasce da questi nove atleti. Il difetto **residuo**, isolato e corretto qui, e per un atleta la cui **unica** appartenenza cita un riferimento del genere, senza nessuna riga gemella da cui autoripararsi: `buildClubCategoryOptions` (parametro `athletes`, dietro `getClubCategories()`) creava allora una voce nuova nel catalogo delle categorie, e `buildCategoryGroups` la promuoveva a gruppo **implicito** — selezionabile, indistinguibile a schermo da una squadra vera. `NormalizedCategoryOption` porta ora `configured` (vero per ogni voce da `clubCategories`/`resourceCategories`, falso solo per una voce nata esclusivamente da una fusione "derivata" senza corrispondenza); `buildCategoryGroups` scarta le voci `configured: false` prima di generare un gruppo implicito. **Onesta sullo stato**: nessun atleta di questo club, oggi, ha *solo* la riga corrotta — il pattern trovato si autoripara sempre, per questi nove — quindi questa correzione non e stata *osservata* riprodurre il sintomo esatto dello screenshot di regressione con il dato attuale; e una difesa verificata contro una classe di dato che *produrrebbe* quel sintomo se si presentasse (un import parziale, un rollover incompleto, un ordine di scrittura diverso), non la prova che fosse quella la causa. La riproduzione end-to-end con il dato reale del club (categorie, sedi, `category_groups`, tutti e sei) da `buildCategoryGroups`/`getActiveCategoryGroups` continua a restituire esattamente le quattro voci attese — vedi la sonda in `tests/lib/categoria-fantasma-atleta-derivata.test.mjs` e in `categoria-omonima-programma-allenamenti.test.mjs`. Se la regressione e ancora visibile a schermo dopo un refresh forzato del browser (non solo una ricarica), il sospetto principale resta una cache lato client su un bundle o una risposta precedenti al deploy, non il reader | Se ricompare con un refresh forzato: catturare le risposte di rete (`categories`, `category_groups`) direttamente dal browser nel momento del difetto, non trascritte a mano, per confrontarle byte per byte con quanto verificato qui |
| **D-AUD-41** | Medium | **La struttura consigliata per sede e l'avviso cross-site coprono solo tre delle superfici che creano/modificano un evento** (issue UAT). `resolveRecommendedStructures`/`isCrossSiteEvent` (`src/lib/club-sites.ts`) sono cablate in `AddTrainingForm.tsx` + `src/app/training/page.tsx` (creazione allenamento), `AddMatchForm.tsx` + `src/app/matches/page.tsx` (creazione gara) e `WeeklyTrainingSchedulePanel.tsx` (aggiungi/modifica slot del programma settimanale) — le superfici esplicitamente elencate dal mandato con priorita piu alta (creazione, non modifica; il programma settimanale, che genera in blocco). **Restano fuori**, deliberatamente, per restare in un commit atomico: `EditTrainingForm.tsx` (usa `locations: string[]` solo etichette, senza `structureId`/`siteId`: la modifica di un allenamento esistente non ha oggi la sede della struttura da confrontare) e `src/components/trainer/trainer-event-editor-dialog.tsx` (il dialogo lato allenatore per creare/modificare allenamenti e gare, con un `opzioniLuogo` unfiltered dello stesso tipo gia visto nelle altre superfici prima di questo commit) | Portare `structureId`/`siteId` in `EditTrainingForm.tsx` (oggi solo label), poi applicare lo stesso `resolveRecommendedStructures`/avviso cross-site a entrambe le superfici, con lo stesso guard "solo se la struttura e cambiata" gia in `WeeklyTrainingSchedulePanel.tsx` per non riavvisare un evento gia salvato cross-site |
| **D-AUD-40** | Low | **`ToastProvider` (`src/components/ui/toast-notification.tsx`) espone un valore di contesto nuovo a ogni `showToast`**, il che ri-renderizza ogni consumatore di `useToast()` — inclusa la pagina che ha appena mostrato l'errore. E stato il moltiplicatore (non la causa) del bug UAT "il form Crea categoria perde tutto dopo un errore": `CategoryEditorDialog` si reinizializzava a ogni re-render del genitore innescato dal toast, perche il suo effetto di reset dipendeva da array/oggetti ricreati a ogni render (`initialAssignedTrainerIds`, `availableTrainers` in `src/app/categories/page.tsx`). La correzione (questo commit) chiude il difetto alla fonte in `CategoryEditorDialog.tsx` — il reset ora dipende dal **bersaglio** (`isOpen` + id), non dal riferimento delle prop — quindi il sintomo e chiuso a prescindere da questo debito. Resta pero una classe di bug latente: qualunque altro dialogo che re-inizializzi `useState` da una prop ricreata a ogni render puo essere svuotato dallo stesso pattern (segnalato dall'audit anche su `AddMatchForm.tsx`/`AddCertificateForm.tsx`, oggi protetti da un guard su `initialData`/`certificate?.id` ma non da un `isOpen`+bersaglio esplicito) | Stabilizzare il valore di contesto di `ToastProvider` con `useMemo`/`useCallback`, e applicare lo stesso guard `isOpen` + chiave-bersaglio di `CategoryEditorDialog.tsx` agli altri dialoghi con un effetto di reset totale |

## Debito aperto dal redesign Web V2 (WP-RD, ADR-0184, 2026-09-15)

| ID | Gravita | Cosa | Rimedio |
|----|---------|------|---------|
| **D-RD-1** | Medium | **I blocchi anagrafici condivisi sono ancora V1 dentro i moduli V2.** `PersonIdentityFields`, `PersonResidenceFields`, `AssistedAddressFields`, `PhoneField`, `ClothingSizesFields`, `DocumentExtractionField`, `CertificateAttachmentField`, `AvatarUpload` (usati da nove anagrafiche) rendono con `Input`/`Label` shadcn dentro pannelli e cassetti `Field`/`TextInput` del sistema: nello stesso modulo convivono due aspetti | Migrarli **una volta sola** ai primitivi di `src/components/web/forms/Field.tsx`, non pagina per pagina |
| **D-RD-2** | Low | **Componenti nati accanto alle pagine che una seconda pagina chiedera**: guscio dell'azione di massa guidata (§7.7, in `medical/v2/reminder-bulk-drawer.tsx` e `staff/v2/move-department-drawer.tsx`), `RailPanel` (`dashboard/v2/DayRail.tsx`), `RecordRowList`/`FieldList`/`RecordSection`/`FileInput` (`athletes/profile/v2/record-primitives.tsx`), righe ripetibili nel cassetto e cassetto a modi (`procura/v2/*`), `SiteContextControl` (`athletes/v2/athletes-context-controls.tsx`, importato da training e categorie), KPI a piano 0 (`reports/v2/report-stat-tile.tsx`), `MonthPicker` e il controllo tri-stato delle presenze (`training/v2/*`) | Promuoverli in `src/components/web/` alla seconda richiesta, con un test |
| **D-RD-3** | Low | **Il DataGrid non ha un modo `serverSide` esplicito**: chi filtra sul server (Prima nota, Atleti sopra i 200) usa `apply: () => true` + `onFiltersChange`, e il contatore «N risultati» della ricerca conta le righe della pagina; la paginazione del server vive in `footerRow` (`ServerPager`) con il piede della griglia spento | `serverSide: { total, page, onPageChange }` nel contratto della griglia; `canSelectRow(row)` per le righe non selezionabili (rate pagate) |
| **D-RD-4** | Low | **Etichette di stato costruite fuori da `src/lib/web/status.ts`**: accesso genitore (Account collegato / Token attivo / Token scaduto / Solo recapito), tesseramento «In rinnovo», assegnazioni kit, documenti condivisi (`athletes/profile/v2/record-primitives.tsx`), «In rinnovo»/«Da firmare» nei documenti | Portarle nella mappa unica quando si tocca quella scheda |
| **D-RD-5** | Low | **`/athletes` non legge `?category=`** che «Vedi atleti» di `/categories` (come la V1 morta) passa; e i contatori dell'intestazione riattivano una vista solo se cambia (`requestedViewId` senza nonce) | Far leggere il parametro all'elenco atleti (vista o filtro categoria); un nonce nel contratto di `requestedViewId` |
| **D-RD-6** | Low | **`addStaffMember` in `simplified-db.ts` scarta `clothingSizes`, `birthPlaceCode`, `documentIssueDate`, `residencePermitExpiry`, `age`, `education` alla creazione** — la V1 li perdeva allo stesso modo; il modulo V2 li chiede e li manda | Allineare la funzione di scrittura ai campi del modulo (dominio, non UI) |
| **D-RD-7** | Low | **Prima nota non ha un pulsante di esportazione** (lo ha `/reports` con `accounting.export`) e **nessuna schermata crea/modifica un conto finanziario** (l'API c'e): entrambe le lacune sono V1, dichiarate e non colmate dal redesign | Decisione di prodotto: dove vive l'export della prima nota; una schermata «Conti» sotto Impostazioni |
| **D-RD-8** | Low | **Il redesign staging non ha Stripe**: le variabili sono `sensitive` su Vercel e non copiabili via API (`docs/redesign/DEPLOY.md`) | Chiavi di test dedicate e un webhook proprio, se servono prove del checkout sul redesign |
| **D-RD-9** | Low | **Le pagine con il corpo ancora V1 portano il guscio V2 con la vecchia veste dentro** (titoli, card, tabelle shadcn): coerenti nel funzionamento, non nell'aspetto | La procedura del WP-RD, pagina per pagina |
| **D-RD-10** | Low | ~~Tre copie dello stesso hook nate con la Wave D~~ **Chiuso il 2026-09-15**: `useRouteClubId` + `withClubId` in `src/components/web/hooks/use-route-club-id.ts` sostituiscono `useMemberClubId`, `useStructuresClubId`, `useSponsorClubId` e `useStaffClubId`. Restano: `BOOKING_STATUS`/`BOOKING_PAYMENT_STATUS` (`structures/v2/structure-model.ts`) e gli stati degli incassi sponsor (`sponsors/v2/sponsor-model.ts`) fuori da `src/lib/web/status.ts` (vedi D-RD-12); il dialogo di storno (`sponsors/v2/storno-incasso-dialog.tsx`) e `MonthGrid` (`calendar/v2/MonthGrid.tsx`) candidati alle fondamenta (D-RD-13) | Gli stati nella mappa unica; gli altri promossi alla seconda richiesta |
| **D-RD-11** | Low | **`EventRsvpFields` e `TrainingGroupSelector` restano blocchi V1 dentro i cassetti V2** di gare (`matches/v2/MatchFormDrawer.tsx`, `MultipleMatchesDrawer.tsx`; stessa natura di D-RD-1, dominio eventi) | Migrarli insieme ai blocchi anagrafici di D-RD-1 |
| **D-RD-12** | Low | **Spec di stato locali fuori da `src/lib/web/status.ts`**, una famiglia per pagina della Wave E: documenti (DA VERIFICARE, DA INTEGRARE, APPROVATO…), modulistica (RITIRATO, GENERATO, CONSEGNATO, IN ATTESA DI FIRMA…), consensi (ACCETTATO, RIFIUTATO, REVOCATO, MANCA), appuntamenti e fasce (`secretariat/v2/secretariat-model.ts`, `appuntamenti/v2/slot-model.ts`), comunicazioni (INVIATO/SALTATO/NON RIUSCITO, IN BACHECA, ACCESA/SPENTA, LETTA/DA LEGGERE, RIUSCITA/FALLITA/NEGATA), lavoro sportivo (`sport-work/v2/sport-work-status.ts`, 21 spec), magazzino (`clothing/v2/clothing-model.ts`), club (FUTURA, CLASSIFICATA/DA CLASSIFICARE), recapiti (VERIFICATO/DA VERIFICARE, APERTO) | Promuoverle nella mappa unica per famiglia, con il test di `tests/web/status.test.mjs` |
| **D-RD-13** | Low | **Candidati alle fondamenta emersi dalla Wave E**: `ReasonDialog` (conferma con motivo obbligatorio, `sport-work/v2`), `SuggestInput` (autocomplete §8.2, `secretariat/v2`), rail settimana (`secretariat/v2/agenda-rail.tsx` duplica `training/v2/WeekRail`), `StepperHeader` (`organization/v2/season-manager.tsx`), ~~`AuditPager` = terza copia del pager server~~ (chiusa il 2026-09-15: `ServerPager` in `src/components/web/datagrid/ServerPager.tsx`, con il nome della cosa), ~~`exportGridCsv` riscritta in sette moduli~~ (chiusa il 2026-09-15: `src/lib/web/export-grid-csv.ts`, test `tests/web/export-grid-csv.test.mjs`), `AudiencePicker`, cella d'identita per cose (`ObjectIdentityCell`), `Section` a fisarmonica controllata, `MenuItem` a due righe, campo numerico inline in cella (`clothing/v2/numbering-area.tsx`), `RowActionDef` con sottomenu, `FilterDef` di tipo testo | Una copia sola in `src/components/web/` alla prossima pagina che li chiede (stessa regola di D-RD-2) |
| **D-RD-14** | Low | **`notification-data.ts` (`notifications/v2`) duplica le letture/scritture del cassetto** `NotificationDrawer`, che non le esporta; la finestra fissa di sei giorni della pagina e un comportamento V1 conservato | Un modulo dati solo importato da entrambi; decidere la finestra |
| **D-RD-15** | Low | **`settings.notifications` e `settings.system` si scrivono e nessun modulo le legge** (V1); la revoca di un accesso in Ruoli e accessi ora chiede il nome del club (guideline 08 §8.9) e va confermata come scelta di prodotto; le tre scritture dell'assegnazione kit (`kit_assignments` + `clothing_inventory` + `jersey_assignments`) restano non atomiche come in V1 | Decisioni di prodotto; per il kit, una scrittura sola lato server |
| **D-RD-16** | Medium | **Bonifica delle righe storiche di `athlete_category_memberships` del club pilota** (ADR-0185). Il censimento in sola lettura (`scripts/censimento-appartenenze-legacy.mjs --club 4139ddd1-…`) conta **215** riferimenti con il nome al posto dell'identificativo — 213 righe (211 gemelle di una riga identificata dello stesso atleta, 1 primaria senza gemella da aggiornare, 1 «Pulcini» ambiguo per nome che si risolve dopo la primaria) + 2 valori della colonna `athletes.category_id` — tutti `DETERMINISTIC` verso una categoria vera, 0 ambigui, 0 irrisolvibili. Piano di bonifica (regole, conteggi attesi, transazione, backup, dry-run, validazioni, rollback, parita): [`docs/redesign/D-RD-16-piano-bonifica-appartenenze.md`](../redesign/D-RD-16-piano-bonifica-appartenenze.md); lo strumento e `scripts/bonifica-appartenenze-legacy.mjs` (dry-run per default, scrittura solo con tutte le guardie del piano §8, un branch ammesso solo), con le regole pure in `scripts/lib/bonifica-appartenenze.mjs` provate su fixture; dry-run del 2026-09-15 sul redesign: UPDATE 1, DELETE 212, colonne 2, R1 165 / R2 47 / R3 1 / R0 0, V1–V5 OK, idempotente, ritorno esatto; copia di sicurezza `br-soft-bread-al8kcvmh`; **eseguito sul redesign il 2026-09-16** (run `20260916-fortitudo-write-1`: 428 → 216 righe, 0 fuori catalogo, audit 215 righe, ritorno pronto; piano §14). Fasi C (46 nomi stantii) e B (213 proiezioni `athletes.data`) eseguite sul redesign il 2026-09-16 con `scripts/bonifica-appartenenze-fasi.mjs` (piano §15): sul redesign D-RD-16 e **chiuso**. **Non eseguito sullo staging Fortitudo** (fasi A, C, B): serve un'altra autorizzazione e l'aggiunta del branch a `BRANCH_AMMESSI`. Il lettore non ne fa piu categorie ne secondarie fantasma, e il salvataggio della scheda fonde la gemella al primo salvataggio dell'utente; le righe pero restano in archivio finche nessuno le tocca | Una migrazione dedicata (`UPDATE`/`DELETE` per riga, con il censimento come input e l'audit come uscita), eseguita **solo con autorizzazione esplicita** (CLAUDE.md §8); prima su `web-redesign-staging`, poi sullo staging Fortitudo |
| **D-RD-17** | Low | **CHIUSO il 2026-09-16 (ADR-0186)**: (a) l'indice scrive `UNKNOWN_CATEGORY_LABEL` per un riferimento nudo sconosciuto con il catalogo in mano e `resolveCategoryLabelForTraining` passa dall'indice; (b) import, modulistica e data di nascita non scelgono fra due omonime; (c) `MembershipRoleBadge`; (d) un indice per card. In piu il vaglio del server (`category-write-guard.ts`) e il writer client con il catalogo: writer capaci di scrivere un'etichetta come `category_id` = 0. Voce storica: **Residui di etichetta fuori dall'indice dopo ADR-0185** (revisione ostile, Medium/Low non chiusi): (a) `resolveCategoryLabelForTraining` (`training-utils.ts`) e `resolveCategoryLabel` (`category-utils.ts`) restituiscono il riferimento **com'e** quando il catalogo non lo conosce — per un club con i soli nomi e giusto, per un club con gli identificativi e un `category-…` a schermo (dashboard V2 «oggi», riga della pagina Allenamenti); senza catalogo non si distingue un nome da un identificativo, e la scelta di prodotto e aperta; (b) l'import atleti (`athletes/page.tsx`, `categoryIdByKey`) risolve l'etichetta di una riga con una mappa per nome a ultimo-vince: con due «Pulcini» una riga importata «Pulcini» finisce sull'ultima in silenzio; (c) i chip «· Primaria» / «· Secondaria» usano lo stesso glifo della sede («Pulcini · Scauri · Primaria»); (d) `AthleteProfileSections`/`AthleteActivitySections` ricostruiscono l'indice per chip invece di riceverlo | (a) decidere: `UNKNOWN_CATEGORY_LABEL` quando il catalogo esiste ed e non vuoto, il riferimento com'e solo senza catalogo; (b) `resolveCategoryId` (gia sicuro sull'ambiguita) e riga segnalata invece di assegnata; (c) una pillola separata per il ruolo; (d) passare `index` |
| **D-RD-19** | Low | **Pannelli a fondo scuro dei pagamenti e dei voucher** (`src/components/payments/EnrollmentPaymentBreakdown.tsx`, `InstallmentLedgerList`, `RefundDialog`, `DocumentDecisionDialog`, `funding/FundingPeriodsTable`, `AthleteFundingSummary`, `SettleAccrualDialog`): testo chiaro su `slate-900/950`, la sola famiglia fuori dai token dopo ADR-0187. Non e testo scuro su scuro (leggibile), ma e un secondo linguaggio dentro pagine V2 | Il WP «pannelli pagamenti V2» gia aperto in WP-RD: `Panel` chiaro con `InsetBlock`, niente fondo scuro |
| **D-RD-20** | Low | **`/private/*`** (console di piattaforma, anteprima email, api-docs) resta sulla tavolozza legacy: non e raggiungibile da un ruolo di club e non e nei mockup | Un lotto suo quando il design la copre |
| **D-RD-22** | ~~Low~~ **chiuso** (2026-09-16, secondo lotto) | ~~La conversione di una persona in prova non e una transazione~~ — `createResource` accetta la transazione di chi chiama (`ResourceRequestOptions.client`) e `convertTrialAthlete` scrive presa, scheda, appartenenze e collegamento in **un solo** `prisma.$transaction`; gli identificativi delle appartenenze si coniano prima, cosi la proiezione nasce con le righe vere e non serve una seconda scrittura. Provato sul database vero con sei «Converti» simultanei (`scripts/prova-conversione-concorrente.mjs`: una scheda, una prova collegata, le altre cinque rifiutate) e sul fake (prove 43–44). **Coda (ADR-0194, UAT):** con una categoria la conversione cadeva sul vaglio del padre, che leggeva la scheda con il client globale invece che dalla transazione; chiuso il 2026-09-16, prova 45 e lo stesso script con `categoryId` | — |
| **D-RD-23** | Low | **Pratiche di iscrizione, rinvii dichiarati** (ADR-0189/0191/0192): gli allegati non entrano in bozza (si caricano all'invio); nessun OTP sul recapito prima dell'invio o della ripresa (gettone a 256 bit e scadenza); nessuna retention automatica delle pratiche chiuse e delle bozze scadute (solo `purgeExpiredFormDrafts` a chiamata); cancellazione delle bozze per recapito non implementata; nessun avviso del builder su campi personalizzati «allergie/patologie» senza casella esplicita (art. 9) | Decisioni del club/DPO; un cron per la retention |
| **D-RD-24** | Low | **Firma e conservazione** (ADR-0192): la firma disegnata e un'evidenza grafica (FES); FEA/FEQ, marca temporale qualificata e conservazione a norma AgID richiedono un fornitore e un'astrazione `SignatureProvider` disegnata e non implementata; il PDF resta del browser (D38) | Confronto fornitori QTSP in un lotto separato |
| **D-RD-25** | Low | **Appartenenze senza validita temporale** ([ADR-0194](18-decision-log.md) §6): `athlete_category_memberships` e lo stato corrente; il contesto storico si legge dall'evento e dalla fotografia `is_extra_category` sulla riga di partecipazione. Restano calcolati sull'appartenenza corrente: i **tassi** del rapporto per categoria (denominatore = tutti gli eventi della categoria nel periodo, anche prima dell'ingresso), le ore dei **contributi pubblici** (filtro sui gruppi correnti, regola anti-frode di `funding-multisite`: un cambio di gruppo a meta periodo sposta le ore), la distinzione primaria/secondaria sugli appelli passati (la fotografia e un booleano: `member`) | Date di inizio/fine sulle appartenenze, con migrazione additiva e un ADR suo |
| **D-RD-26** | Info | **Dato del pilota creato dall'UAT dell'utente il 2026-09-16 (11:39–11:53Z)** con il cassetto precedente ad ADR-0194: dodici righe `Aquilotti [S]` spurie (Izzo, Luison, Casilli, Cardillo, Buonamano, Parasmo, Vozzolo, Cardile, Santorsola, Del Prete, Di Russo, Serpe Scala, `category-1787322218340-0h1e2r5` primaria + `Aquilotti` secondaria) e quattro coppie `Pulcini (category-1787322040142-mbawy4c) + S. Cosma` non configurate (Romano, Pedico, Rossini, Intili); due Pulcini senza sede (Migliaccio, Forte). Il writer nuovo non le produce piu; le righe passano com'erano e si sistemano con «Cambia categoria» o con una bonifica **autorizzata** | Nessuna scrittura senza autorizzazione (CLAUDE.md §8) |
| **D-RD-29** | Medium | **Il versamento F24 del lavoro sportivo crea un movimento manuale senza stagione** (`src/lib/server/sport-work-agenda.ts`, `payObligation` → `createAccountingEntry` senza `seasonId`/`activeSeasonId`): la riga si attribuisce per data, cioe in due stagioni sovrapposte (revisione ADR-0196 B2). La rotta del lavoro sportivo deve passare `x-active-season-id` come fa `accounting/entries` | **Chiuso in ADR-0197 §11**: la rotta `obligations/:id/complete` passa la stagione dichiarata al versamento (`payment.activeSeasonId`), con le regole di `createAccountingEntry` |
| **D-RD-30** | Medium | **Le righe storiche di `clubs.transactions` dichiarano `seasonId` e la vista `accounting_ledger_lines` le proietta con `season_id = NULL`** (migrazione `20260909170000_…`, ramo `legacy-transaction`): la prima nota le attribuisce per data ignorando la stagione scritta (revisione ADR-0196 B6). Serve una migrazione della vista che proietti `NULLIF(btrim(value->>'seasonId'), '')` | **Chiuso in ADR-0197 §11**: migrazione `20260917120000_adr0197_stagione_dei_movimenti_storici` (sola vista), provata in transazione annullata sul database del redesign |
| **D-RD-31** | Low | **Incassi e rate non hanno `season_id`** (`payment_transactions`, `athlete_payments`): la loro stagione e quella del piano o della data, e l'eliminazione di una stagione le conta solo attraverso i piani della stagione (ADR-0197 §9). Un incasso registrato senza piano in una stagione sovrapposta non e attribuibile per identita | Aperto: una colonna e un backfill per data unica, in un lotto proprio |
| **D-RD-32** | Low | **Gli eventi senza stagione nati prima di ADR-0197** (sul pilota: 3 allenamenti e 1 gara creati a mano) appartengono alla stagione piu vecchia per regola, non per dichiarazione. Il backfill per finestra unica e possibile ma non e stato fatto: dato reale, e la regola gia li mostra dove stanno | Aperto: solo su autorizzazione, con la finestra unica come criterio |
| **D-RD-33** | Low | **Il selettore di consultazione di una stagione non attiva non esiste**: per leggere la A si attiva la A (ADR-0197 §3). Il server distingue gia «dichiarata» da «attiva», quindi un selettore per browser non cambia nessuna regola lato server | Aperto: decisione di prodotto |
| **D-RD-34** | Low | **Un record di un'altra stagione tolto da un client che non la mostra viene conservato dal server** (`preserveOtherSeasonRecords`, ADR-0197 §5): la colonna intera rimandata senza quel record lo rimette, perche fuori dal perimetro. Si toglie nella stagione in cui si guarda | Aperto: esprimere le cancellazioni per identificativo |
| **D-RD-35** | Medium | **Una chiamata al registro generico senza `x-active-season-id` non ha perimetro di stagione** (mobile, script): legge tutte le stagioni e scrive senza stagione. Il risolutore canonico dice «assente → attiva», ma applicarlo al registro cambia il contratto del mobile (ADR-0025, sviluppo differito) | Aperto: decisione di prodotto con il mobile |
| **D-RD-36** | Medium | **«Rimuovi programmati»/«Rigenera da capo» vedono solo la stagione mostrata mentre il conflitto di campo e trasversale**: con due stagioni sovrapposte e gli allenamenti generati nella vecchia, la nuova genera solo conflitti e non puo toglierli (revisione ADR-0197 B7) | Aperto: opzione «anche le altre stagioni» + stagione dell'evento occupante nel conflitto |
| **D-RD-37** | Medium | **L'import atleti risolve gli omonimi sul catalogo di tutte le stagioni** (`loadClubCategoryCatalog`): «Under 15» esiste gia nella stagione scorsa e l'unico collegamento offerto e quello (revisione ADR-0197 A-M7) | Aperto: filtrare il catalogo sulla stagione dichiarata |
| **D-RD-38** | Medium | **Riporto, conteggio e distacco delle assegnazioni non coprono gli `staff_members` con ruolo allenatore** (revisione ADR-0197 D-L1): i loro riferimenti restano dangling dopo l'eliminazione di una stagione | Aperto: includere `isTrainerLikeStaffMember` nelle tre routine |
| **D-RD-39** | Low | **Il prefiltro in archivio degli omonimi non toglie gli accenti in mezzo alla parola** (ADR-0198 §5): il confronto in memoria (`nameMatchKey`) e giusto, ma «Sébastien» digitato «Sebastien» puo non arrivare dal `contains` | Aperto: `unaccent` o colonna normalizzata |
| **D-RD-40** | Low | **I rapporti sommano la copia `attendance` del payload alle righe di `club_event_participants`** (`club-report-utils`, `category-athlete-stats`, `athlete-category-analytics`): un evento migrato puo contare due volte (revisione ADR-0198 B11) | Aperto: leggere solo le righe |
| **D-RD-41** | Low | **Cron quotidiano con `generateDaysAhead = 7` senza margine** (revisione ADR-0198 D9): un ritardo del cron oltre l'ora della corsa precedente lascia scoperte le fasce di prima mattina | Aperto: minimo `> intervallo del cron` o arrotondare la finestra del cron a fine giornata |
| **D-RD-42** | Low | **Omonimi e candidati della conversione senza perimetro ADR-0103** per chi puo convertire (revisione ADR-0198 C13): l'identita e del club, ma va deciso e scritto | Aperto |
| **D-RD-43** | Low | **Pannello del programma settimanale e generatore risolvono i nomi di categoria su cataloghi diversi** (stagione vs club, revisione ADR-0198 A4): un nome ambiguo fra due annate puo dirsi «Assegnato» a schermo e non esserlo in generazione; la diagnostica lo dice | Aperto: catalogo intero al client o sola risoluzione per id |
| **D-RD-44** | Low | **Un allenatore sospeso non si deriva sugli allenamenti nuovi ma, se scritto sulla voce ed assegnato, resta** (revisione ADR-0198 A11) | Aperto: la sospensione come fatto del dominio eventi |
| **D-RD-28** | Medium | **Cataloghi filtrati per stagione passati a `normalizeAthleteCategoryMemberships`** fuori dalla pagina Atleti (scheda atleta, editor delle appartenenze, superfici allenatore): la funzione vuole il catalogo intero, e con il sottoinsieme della stagione attiva un'appartenenza a una categoria archiviata ripiega sul nome e si traveste da squadra corrente — che sull'editor puo diventare una scrittura. La pagina Atleti e stata corretta in ADR-0196 (secondo catalogo senza `x-active-season-id`); le altre superfici vanno censite e allineate | Aperto |
| **D-RD-27** | Medium | **Il `DELETE` di un'anagrafica dal registro generico si audita come `anagrafica.updated`** (`auditResourceWrite` in `src/app/api/v1/[resource]/[id]/route.ts` rimappa ogni successo sulle risorse anagrafiche), con `metadata` vuoto: una cancellazione in blocco di 300 schede dalla pagina Atleti e indistinguibile da 300 modifiche. Visto ricostruendo il 2026-09-16 sul pilota (ADR-0195, forense). Serve un'azione `anagrafica.deleted` (o conservare `resource.deleted`) e un metadata con il nome della persona | **Chiuso in ADR-0197 §10**: `anagrafica.deleted` con `operation`, `resource` e `label` della scheda; la modifica resta `anagrafica.updated` |
| **D-RD-21** | Info | **Proiezioni `athletes.data.categoryMemberships` gia derivate prima di ADR-0187 §9** sul club pilota: il censimento D-RD-16 ne conta 1 (atleta R3, modifica dell'utente del 2026-09-16 02:05Z: secondaria tolta, sede azzerata; 215 righe, era 216). Il writer non ne produce piu; quella riga si riallinea al prossimo salvataggio della scheda o con una bonifica di fase B **autorizzata** | Nessuna scrittura senza autorizzazione (CLAUDE.md §8) |
| **D-RD-18** | Low | **`buildAthleteParticipationAnalytics` (`src/lib/athlete-participation-utils.ts`) e codice irraggiungibile** con un risolutore di etichette locale per nome (`resolveCategoryLabel` interno, `buildCategoryLookup` per nome). Nessuna schermata lo chiama (verificato con ADR-0186): non mostra niente, ma e una seconda stesura della regola dell'etichetta che aspetta di essere riusata | Toglierlo, o farlo passare dall'indice canonico se un consumatore nasce |
| **D-RD-54** | Medium | **Un evento datato solo (senza orario) puo essere escluso dal consuntivo per un'ora o due dopo la mezzanotte locale** (`isWithinActivityWindow`, `src/lib/athlete-category-analytics.ts`, revisione ostile Wave F, Reviewer A): `new Date("YYYY-MM-DD")` ancora al giorno a mezzanotte **UTC**, che per un club in UTC+1/+2 (l'Italia) cade 1-2 ore **dopo** la mezzanotte locale. Un allenamento di oggi, controllato nella prima ora e mezza dopo la mezzanotte locale, risulta «futuro» rispetto a `now` e sparisce dal conteggio per quella finestra — si autocorregge da solo appena l'orologio UTC raggiunge la mezzanotte | Confrontare `now`/`activityStartAt` sulla cornice civile del club (`civilDateOf`, gia usata altrove nel dominio eventi), non su `new Date()` nudo |
| **D-RD-55** | Medium | **La finestra `now`/`activityStartAt` (mandato multi-stagione B5/B6) non e applicata sulla bacheca dell'allenatore**: `trainer-athlete-profile-page.tsx` chiama `calculateAthleteCategoryAnalytics` senza passare ne `now` ne `activityStartAt` (la scheda atleta dell'ufficio li passa). Per lo stesso atleta, ufficio e allenatore possono vedere `trainingsTotal`/`attendanceRate` diversi quando esistono allenamenti futuri gia in calendario o eventi precedenti all'ingresso | Passare `now`/`activityStartAt` anche dalla bacheca allenatore, stessa chiamata |
| **D-RD-56** | Medium | **`buildCategoryCompatibilityIndex` (`src/lib/category-compatibility.ts`) indicizza sia per identificativo sia per nome nella stessa mappa**: due categorie omonime di stagioni diverse collidono sulla chiave-nome, l'ultima letta vince. Non riguarda la scelta diretta di una categoria (sempre per id), ma un `NumberingGroup.categoryIds`/`compatibleCategoryIds` scritto per nome (dato legacy o configurazione a mano) puo risolversi sulla categoria sbagliata | Applicare la stessa disciplina di `resolveCategoryId` (un nome che ne nomina due non ne nomina nessuna) invece di lasciare vincere l'ultimo inserito |
| **D-RD-57** | Medium | **L'assegnazione manuale del numero resta possibile dalla pagina Abbigliamento** (`src/app/clothing/page.tsx`, `src/components/clothing/v2/numbering-area.tsx`, non toccati da questo lotto): un operatore con `canManage` puo ancora assegnare a mano un numero di un gruppo qualunque a un atleta «fuori gruppo», bypassando `resolveNumberingGroupForCategory`. Puo essere una funzione di prestito intenzionale (il modulo la distingue gia concettualmente da «squadra di casa»), ma non e dichiarata come tale nel punto in cui si usa | Confermare con il prodotto se e un prestito voluto; se si, dirlo nell'interfaccia; se no, restringerla come gia fatto per la scheda atleta e «Nuovo atleta» |
| **D-RD-58** | Medium | **Il vaglio di riconciliazione di `syncAthleteEnrollmentInstallmentPayments` non chiude il TOCTOU fra due chiamate concorrenti** (revisione ostile Wave F, Reviewer D): il confronto e aritmetica pura in memoria, le scritture che seguono non sono in una transazione ne hanno un vincolo unico noto in questo repository. Due conferme quasi simultanee dello stesso piano potrebbero passare entrambe il vaglio e scrivere due volte la stessa rata. Il vaglio chiude lo scarto di importo, non la doppia scrittura — gia dichiarato in parte da D-RD-49, qui esplicitato come TOCTOU | Una rotta server con blocco transazionale, o un vincolo unico su `(athlete_id, installmentId)` |
| **D-RD-59** | Low | **Il vaglio di riconciliazione si salta quando `installments.length === 0`** (revisione ostile Wave F, Reviewer D): la funzione esportata `syncAthleteEnrollmentInstallmentPayments` non ricontrolla `expectedTotalAmount` in quel ramo, e cancella comunque le rate pendenti esistenti. Non raggiungibile oggi (l'unico chiamante blocca prima), ma la promessa del commento della funzione («nessuna schermata puo scrivere una schedule che non torna») non vale per questo caso | Ricontrollare anche il ramo vuoto, o dichiarare esplicitamente che spetta al chiamante |
| **D-RD-60** | Medium | **Il controllo MIME dell'import DOCX si salta se il client omette `Content-Type`** (`src/app/api/v1/document_templates/docx-preview/route.ts`, revisione ostile Wave F, Reviewer E): un client che manda un file `.docx` senza dichiarare il tipo supera il controllo con la sola estensione. Non e la difesa che conta — un contenuto che non e un archivio OOXML valido non produce niente dentro `convertDocxToHtml`, quale che sia il MIME — ma indebolisce un controllo dichiarato | Valutare se irrigidire il controllo (rischio: respingere client legittimi che non dichiarano il tipo) o lasciarlo com'e e contare solo sul vaglio di contenuto |
| **D-RD-61** | Low | **Nessun tetto sul numero di voci di un `.docx`** (`src/lib/server/docx-import.ts`, revisione ostile Wave F, Reviewer E): il vaglio della bomba d'archivio (Wave F, ora reale: decomprime e conta) limita la dimensione totale, non il numero di voci. Un archivio con moltissime voci minuscole resta comunque limitato dal tetto di 10 MB sul file caricato, quindi bloccato nella pratica, ma non da un vincolo dedicato | Un tetto esplicito sul numero di voci (poche decine per un `.docx` vero), indipendente dalla dimensione |
| **D-RD-62** | Low | **La sanificazione degli entita XML del `.docx` si affida a `@xmldom/xmldom` (dipendenza transitiva di `mammoth`), non a una configurazione esplicita in questo codice** (revisione ostile Wave F, Reviewer E): verificato che la versione installata non espande entita esterne, ma non c'e un test che blocchi questo comportamento se una futura versione lo cambiasse | Un test che alimenta un `.docx` con un `&lt;!ENTITY&gt;`/`&lt;!DOCTYPE&gt;` e verifica che non viene espanso ne genera accesso esterno |
| **D-RD-64** | Medium | **Il riconoscimento della tuta nel catalogo abbigliamento puo scambiare un pantalone per una tuta** (`resolveItemSizeSource`, `src/lib/clothing-delivery.ts`, revisione ostile Wave F, Reviewer C): il tipo di un articolo e testo scritto a mano, e un capo chiamato «Pantaloni tuta» (grafia italiana comune per un pantalone da tuta) contiene la parola «tuta» come parola isolata e viene letto come `tracksuit`, non `pants` — solo per articoli **senza** `sizeSource` esplicito. Effetto limitato a un suggerimento di taglia nella consegna (mai scritto sull'anagrafica, mai sul magazzino): un operatore vede una taglia proposta che puo non essere quella giusta, e la corregge prima di salvare | Impostare `sizeSource` esplicito sul catalogo per i capi ambigui, o affinare il riconoscimento (es. «pantalon» prima di «tuta» quando compaiono insieme) |
| **D-RD-63** | Low | **La deduplica di `assertRegistrationDates` (impronta `federationId::issueDate::expiryDate`) puo far scrivere due volte la stessa riga gia invalida** (revisione ostile Wave F, Reviewer E): non permette una combinazione **nuova** invalida, ma un client puo ripetere la stessa riga storica gia scorretta senza essere bloccato — igiene dei dati, non un modo per aggirare la regola | Deduplicare anche per posizione/identificativo di riga, non solo per valore |
| **D-RD-53** | Low | **Matches, Training e Groups non mostrano le annate nel selettore categoria** (mandato multi-stagione A1/A2): usano `describeCategoryGroupOptions` (`src/lib/club-sites.ts`), un presenter diverso da `buildCategoryDisplayIndex`/`buildMembershipTargetIndex`, mai esteso con `optionLabel`/annate. Non e una regressione di questo lotto (non le mostravano nemmeno prima): stessa famiglia del debito gia dichiarato per Reports/Import (D-RD-45) | Estendere `describeCategoryGroupOptions` con le annate, o farlo passare dall'indice canonico |
| **D-RD-45** | Low | **Il filtro categoria dei Report** (`report-context-controls.tsx`) e il **mapping categorie dell'import atleti** (`AthleteImportDialog.tsx`/`src/lib/athletes/import/plan.ts`) costruiscono l'etichetta categoria per conto proprio invece di passare da `buildCategoryDisplayIndex`/`buildMembershipTargetIndex` (mandato multi-stagione A1/A2, Wave A 2026-09-21): due «Under 15» di sedi diverse restano identiche nel filtro Report, e le annate non compaiono in nessuno dei due | Aperto: allineare i due selettori all'indice canonico in un lotto dedicato |
| **D-RD-46** | Info | **Non esiste un'eliminazione di una persona in prova** (nessuna `DELETE`, nessun writer in `trial-athletes.ts`): l'unica strada e lo stato «non prosegue» (`setTrialAthleteStatus`). Il mandato multi-stagione A5 la presuppone; non implementata perche una `DELETE` con `trial_attendances` gia collegate e una decisione sul dominio (cascata? blocco?) che nessun documento specifica | Richiede una decisione del club prima di scrivere il writer |
| **D-RD-47** | Medium | **`jersey_assignments` non ha una guardia di concorrenza reale** (mandato multi-stagione B13): e un array JSON su `Club`, scritto con un controlla-poi-scrivi lato client (`canAssignNumber` su uno stato appena riletto, poi `updateClubData` sull'intera colonna). Due operatori che assegnano lo stesso numero nello stesso istante non sono bloccati dal database — solo un'etichetta «Duplicato» dopo il fatto | Una rotta server con blocco transazionale per `(clubId, groupId)`, non un vincolo sulla colonna JSON |
| **D-RD-48** | Low | **Il rapporto per categoria dell'allenatore** (`src/lib/category-athlete-stats.ts`, via `trainer-athletes-dashboard-page.tsx:455-462`) **chiama `calculateCategoryAthleteStats` con `attendance: []`**, la stessa radice gia in D-RD-40: legge solo la copia `training.attendance` incassata nella proiezione, mai `club_event_participants`. La scheda atleta singola e stata corretta (Wave B, mandato multi-stagione B3/B4); questo rapporto multi-atleta no, perche era fuori dallo scope dichiarato di quella wave | Stesso schema della scheda atleta: leggere `club_event_participants` per gli atleti/eventi della categoria e passarli come `attendance` |
| **D-RD-49** | Medium | **Il vaglio di riconciliazione delle rate (mandato multi-stagione D12-D15) vive nell'unico orchestratore del flusso** (`syncAthleteEnrollmentInstallmentPayments`, client), **non in un confine di rete**: le singole scritture passano gia dal registro generico autorizzato (`POST`/`PATCH /api/v1/simplified_payments...`), ma quel registro autorizza riga per riga senza sapere che un gruppo di righe deve sommare a un totale. Chi chiamasse il registro generico direttamente, scrivendo le rate una per una senza passare da questa funzione, non incontrerebbe il vaglio | Una rotta server dedicata (`POST /api/v1/athletes/:id/enrollment-plan`) che materializzi l'intera schedule in una transazione, con la riconciliazione come guardia server autentica — cambio di architettura del writer, non un vincolo aggiunto |
| **D-RD-50** | Low | **`AthletePayment.amount`/`PaymentTransaction.amount` restano `Float`**, non `Decimal`/centesimi interi (mandato multi-stagione D3), per scelta esplicita: migrare una colonna finanziaria gia popolata con dati reali e un'operazione sui dati, non una wave di codice. I confronti passano gia da centesimi (`toCents`), che mitiga ma non elimina il rischio di un valore `Float` scritto direttamente altrove | Una migrazione dedicata, con backup e autorizzazione esplicita (CLAUDE.md §8), fuori da questo batch |
| **D-RD-51** | Low | **L'import DOCX (mandato multi-stagione E5-E12) non porta le immagini incassate nel documento**: `convertDocxToHtml` le conta e le dichiara in `unsupported`, ma non le carica come allegati ne riscrive `src`. La sanificazione esistente (ADR-0190) le toglierebbe comunque se arrivassero come `data:` URI, quindi la scelta e sicura, solo incompleta | Caricare ogni immagine incassata come allegato vero (`src/lib/server/attachments.ts`) e riscrivere `src` sull'endpoint del sistema, con un tetto al numero/peso delle immagini per documento |
| **D-RD-52** | Info | **PDF e ODT restano fuori dall'import Modulistica** (mandato multi-stagione E11): un PDF e un documento di riferimento statico che questo dominio non rende editabile senza un OCR/estrattore dedicato; un ODT userebbe un parser diverso da `mammoth`. Nessuno dei due e un blocco tecnico di questa wave, sono decisioni di prodotto rimandate | Valutare separatamente se e quando il club chiede l'uno o l'altro |

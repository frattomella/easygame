...
|------|------|-------------------------------|
| **PP05-D1** | **Un utente solo-OAuth non puo aggiungere il cellulare, ne cambiare email, ne impostare una password.** `createOAuthBootstrapUser` scrive una password casuale che nessuno conosce, e `CURRENT_PASSWORD_REQUIRED` (`src/app/api/v1/auth/user/route.ts`) blocca tutti e tre i campi. La regola di prodotto «il numero e obbligatorio» **non si applica** a quell'intera popolazione, e non c'e modo di rimediare dall'interfaccia. Trovato dalla revisione ostile (MEDIUM-6), riprodotto | Distinguere «non ha mai avuto una password» da «ha una password che non ricorda» pretende una colonna (`password_set_at` o simile), quindi una migrazione e una decisione su cosa un solo-OAuth possa fare con la sola sessione. E un ADR, non una riga. **Nel frattempo la strada esiste ed e «Password dimenticata»**, che imposta una password e sblocca tutto |
...
| **PP05-D6** | **`npm run lint` esce con codice 1 in ogni worktree**, per un conflitto del plugin `@next/next` fra `.eslintrc.json` del worktree e quello identico della radice del repository — che ESLint trova risalendo l'albero, perche i worktree vivono sotto `.claude/`. Non e un errore di codice: `npx eslint --no-eslintrc --config .eslintrc.json` sugli stessi file da **0 errori e 34 warning**, nessuno dei quali nei file toccati | Si chiude con una riga (`"root": true` in `.eslintrc.json`) che vale per tutte e tre le lane parallele e va decisa in integrazione, non dentro una di esse |

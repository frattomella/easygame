...
| **D-RD-22** | Low | **La conversione di una persona in prova non e una transazione** (ADR-0188): tre scritture dal registro generico (scheda, appartenenza, riga di prova). La concorrenza e coperta dalla presa condizionata su `converted_at` e dal rilascio in caso di errore; un crash fra la seconda e la terza scrittura lascia una scheda creata e una prova con `converted_at` e senza `athlete_id` («in conversione»), riconoscibile e non duplicabile | Un `createResource` transazionale, o le tre righe con Prisma dentro `$transaction` quando il registro generico sapra ricevere un client |

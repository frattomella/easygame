...
| **W2-03** | **`saveTrainingAttendance` cancella ancora a mano le righe duplicate** | `src/lib/simplified-db.ts` (~riga 3390) cerca le righe gemelle di `training_attendance` e ne cancella le eccedenti. Con la chiave unica ora in base dati (ADR-0086) quella difesa e **morta**, ma resta un `DELETE` su righe che adesso possono portare una risposta della famiglia. Non e pericoloso oggi — non ci sono piu duplicati da trovare — ed e esattamente il genere di codice che diventa pericoloso quando qualcuno lo modifica senza sapere perche c'era |

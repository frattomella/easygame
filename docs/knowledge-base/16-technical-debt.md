...
| **D-RD-10** | Low | **Tre copie dello stesso hook nate con la Wave D**: `useMemberClubId` (`soci/v2/use-member-club-id.ts`), `useStructuresClubId` (`structures/v2/use-structures-club-id.ts`) e `useSponsorClubId` (`sponsors/v2/use-sponsor-club-id.ts`) leggono tutti il club dalla rotta con lo stesso ripiego; inoltre `BOOKING_STATUS`/`BOOKING_PAYMENT_STATUS` (`structures/v2/structure-model.ts`) e gli stati degli incassi sponsor (`sponsors/v2/sponsor-model.ts`) vivono fuori da `src/lib/web/status.ts`, e il dialogo di storno (`sponsors/v2/storno-incasso-dialog.tsx`) e la griglia mensile (`calendar/v2/MonthGrid.tsx`, sul tipo dell'evento) sono candidati alle fondamenta | Un `useRouteClubId` solo in `src/components/web/hooks/`, gli stati nella mappa unica, gli altri promossi alla seconda richiesta (stessa regola di D-RD-2) |

...
| **D-RD-27** | Medium | **Il `DELETE` di un'anagrafica dal registro generico si audita come `anagrafica.updated`** (`auditResourceWrite` in `src/app/api/v1/[resource]/[id]/route.ts` rimappa ogni successo sulle risorse anagrafiche), con `metadata` vuoto: una cancellazione in blocco di 300 schede dalla pagina Atleti e indistinguibile da 300 modifiche. Visto ricostruendo il 2026-09-16 sul pilota (ADR-0195, forense). Serve un'azione `anagrafica.deleted` (o conservare `resource.deleted`) e un metadata con il nome della persona | Non toccato in questo lotto |

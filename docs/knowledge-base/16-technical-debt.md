...
| **D-RD-16** | Medium | **Bonifica delle righe storiche di `athlete_category_memberships` del club pilota** (ADR-0185). Il censimento in sola lettura (`scripts/censimento-appartenenze-legacy.mjs --club 4139ddd1-…`) conta **215** riferimenti con il nome al posto dell'identificativo — 205 righe gemelle + 9 righe e 1 colonna di un atleta mai migrato — tutti `DETERMINISTIC` verso una categoria vera, 0 ambigui, 0 irrisolvibili. Il lettore non ne fa piu categorie ne secondarie fantasma, e il salvataggio della scheda fonde la gemella al primo salvataggio dell'utente; le righe pero restano in archivio finche nessuno le tocca | Una migrazione dedicata (`UPDATE`/`DELETE` per riga, con il censimento come input e l'audit come uscita), eseguita **solo con autorizzazione esplicita** (CLAUDE.md §8); prima su `web-redesign-staging`, poi sullo staging Fortitudo |

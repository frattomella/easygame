...
| **PP03-D5** | `athletes.data` resta protetto da un elenco di campi **vietati** (`CLINICAL_ATHLETE_FIELDS`), mentre `medical_certificates.data` e passato a un elenco di **ammessi** ([ADR-0125](18-decision-log.md)). La stessa obiezione vale per entrambe: su una colonna JSON libera un elenco di vietati e una scommessa sui nomi che qualcuno usera, e un club che scrivesse `data.patologiaCardiaca` la vincerebbe | Invertire il verso su `athletes.data` vuol dire enumerare cio che il prodotto ci scrive da anni — recapiti, taglie, note organizzative, consensi, iscrizioni, gruppi, tutori — e ogni campo dimenticato sparisce da una schermata. Dentro il certificato l'inversione era gratuita perche **niente** e legittimamente non clinico salvo la provenienza; nell'anagrafica quasi tutto lo e. Va fatto con l'inventario dei campi realmente scritti, ricavato dall'archivio e non dal sorgente |

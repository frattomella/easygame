...
| `SMS_PROVIDER` | Nessun trasporto SMS. La verifica del telefono **non blocca** l'accesso — pretendere cio che non si puo consegnare chiuderebbe fuori ogni account nuovo — ma il numero si chiede lo stesso, perche raccogliere un dato non richiede un contratto (PP-05, ADR-0114/0115). `/api/v1/auth/providers` lo dichiara con `phoneVerificationRequired`, e la schermata di registrazione lo scrive sotto il campo |
...
| `AUTH_REQUIRE_PHONE_VERIFICATION` | Vale `"true"`: la verifica del telefono **e** richiesta, e si spegne solo per scelta esplicita. Da sola non blocca niente — il blocco pretende **anche** un trasporto che consegni (`SMS_PROVIDER`, sopra) e un numero gia in archivio: una regola non puo pretendere cio che l'ambiente non puo consegnare (PP-05, ADR-0115) |

...
| `SMS_PROVIDER` | Nessun trasporto SMS. La verifica del telefono **non blocca** l'accesso — pretendere cio che non si puo consegnare chiuderebbe fuori ogni account nuovo — ma il numero si chiede lo stesso, perche raccogliere un dato non richiede un contratto (PP-05, ADR-0131/0115). `/api/v1/auth/providers` lo dichiara con `phoneVerificationRequired`, e la schermata di registrazione lo scrive sotto il campo |
| `AUTH_OTP_SECRET` | Le impronte OTP ricadono su `AUTH_RATE_LIMIT_SECRET`, poi `CRON_SECRET`, poi `DATABASE_URL`. **Su staging e in produzione va dichiarato un valore proprio**: chi porta via un dump ha quasi certamente anche la stringa di connessione con cui l'ha estratto, e con quella il milione di codici a sei cifre torna enumerabile (vedi 14 — Sicurezza) |
| `AUTH_REQUIRE_PHONE_VERIFICATION` | Vale `"true"`: la verifica del telefono **e** richiesta, e si spegne solo per scelta esplicita. Da sola non blocca niente — il blocco pretende **anche** un trasporto che consegni (`SMS_PROVIDER`, sopra) e un numero gia in archivio: una regola non puo pretendere cio che l'ambiente non puo consegnare (PP-05, ADR-0132) |
| `AUTH_DEFAULT_COUNTRY_CODE` | Vale `+39`. E il prefisso applicato a chi scrive un numero **senza** prefisso; un numero che ne porta uno passa dal suo. Nessun numero resta in forma locale in archivio: la normalizzazione in E.164 e in `src/lib/auth/phone-number.ts` e non e opzionale, perche i contatori per destinatario contano sul numero canonico |

...
| `AthleteCategoryMembership` | `athlete_category_memberships` | Multi-categoria per atleta. Unique `(organization_id, athlete_id, category_id)`, flag `is_primary` (al piu una per atleta e club: indice parziale `athlete_category_memberships_single_primary_per_athlete`), `site_id?` (sede in cui l'atleta svolge **quella** categoria, [ADR-0038](18-decision-log.md)). **Authority della sede corrente** ([ADR-0194](18-decision-log.md)): la coppia `(category_id, site_id)` di una riga nuova o cambiata deve essere un gruppo operativo del club (`assertMembershipPlacementIsCanonical`); una riga nuova senza sede su una categoria con una squadra sola prende quella sede. `athletes.data.siteId`/`site_id` sono copie **deprecate** (writer attivi 0, lettura di ripiego in `getAthleteSiteIds`). Un solo writer: `src/lib/server/athlete-category-memberships.ts` |
| `MedicalCertificate` | `medical_certificates` | `issue_date`, `expiry_date`, `status` |
| `ClubEvent` | `club_events` | **L'evento sportivo: una riga** ([ADR-0098](18-decision-log.md#adr-0098--levento-sportivo-e-una-riga-non-un-elemento-di-un-array)). `kind` — `training` oppure `match` — assorbe allenamenti e gare. `starts_at` e un **istante assoluto** con `timezone` accanto; `legacy_id` conserva l'identificativo della vecchia collezione JSON, unico per `(organization_id, kind, legacy_id)`. `version` per il controllo ottimistico, `capacity`, `rsvp_required`, `rsvp_deadline`, `payload` per cio che non ha ancora una colonna. **`category_id` e la categoria primaria, `category_ids` sono tutte** (PP-01 §A, migrazione `20260903120000_pp01_categorie_evento`): un allenamento multi-categoria ne dichiarava una sola, e le altre vivevano dentro `payload.categories`, dove nessuna query poteva vederle — filtrare per la seconda categoria non trovava l'evento, e l'allenatore di quella categoria era fuori perimetro sul proprio stesso allenamento. `category_ids` e `String[]` e non `Json` perche `hasSome` e un filtro e `payload` no; indice GIN |
...
| `FundingEnrollment` | `funding_enrollments` | Atleta ammesso a un programma: `assigned_amount` (**l'importo assegnato presso questo club**, che non e ne il massimale del bando ne denaro incassato) e `voucher_code`. E il tetto di ogni maturazione e di ogni conferma. Unique `(program_id, athlete_id)`. **Il beneficiario deve appartenere al club del bando**: la colonna non lo impone — `athlete_id` non ha una chiave esterna verso un club — e la verifica sta in `createFundingEnrollment`, perche il maturato legge le presenze del beneficiario |
...
| `FundingSettlement` | `funding_settlements` | Il versamento dell'ente: l'unico momento in cui un contributo diventa denaro. **E anche il movimento bancario del club**: la vista `accounting_ledger_lines` lo proietta con conto, verso e causale, e non se ne scrive un secondo ([ADR-0160](18-decision-log.md#adr-0160--il-bonifico-di-un-ente-e-il-movimento-bancario-non-se-ne-scrive-un-secondo-gli-si-da-una-porta)). Da N15 porta `idempotency_key` (indice unico **parziale** per club), `beneficiary_athlete_id` — nullo quando i beneficiari non sono uno solo — e `description_snapshot`, congelata come l'etichetta della causale |
| `FundingSettlementLine` | `funding_settlement_lines` | La riconciliazione: quanto di un versamento copre quale periodo di quale atleta. **Il liquidato si legge da qui**, non dallo stato del periodo. La riga porta il maturato e non il bando: il legame passa dall`iscrizione, e `createFundingSettlement` verifica che ogni riga appartenga al programma che si sta liquidando |

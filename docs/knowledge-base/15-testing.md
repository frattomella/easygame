...
| `tests/server/document-placeholders.test.mjs` | 22 | **Il risolutore dei segnaposto** ([ADR-0079](18-decision-log.md#adr-0079--il-risolutore-dei-segnaposto-e-lunica-capability-nuova-della-wave-1-e-accetta-quattro-vincoli-per-restarlo)): i dati entrano davvero; `{{payment.total_paid}}` e il **denaro incassato** e non il dovuto di una rata marcata pagata, e uno storno torna indietro anche qui; le rate di un'altra stagione restano fuori; la frequenza arriva dal dominio contributi; un segnaposto sconosciuto e un dato mancante restano bianchi **e vengono elencati**, mai «undefined»; un cognome che contiene `<script>` viene neutralizzato; senza firma il documento esce e lo dichiara; l'atleta di un altro club risponde «Accesso negato» senza far uscire il messaggio dell'ORM |
| `tests/lib/document-placeholder-catalog.test.mjs` | 9 | **Un catalogo solo, un risolutore solo, e la strada vecchia che resta**: in tutto `src/` esiste un unico elenco di segnaposto e un unico risolutore, l'editor consuma quello e non una copia, `renderBlankTemplateForPdf` continua a svuotare i segnaposto del modulo da compilare a mano, e il generatore offre entrambe le strade |
...
| `tests/server/club-signature.test.mjs` | 12 | **Firma e timbro del presidente** (W1-E): l'allegato nasce con `owner_type: "club"` e nelle impostazioni finisce un riferimento e non un data URL, sostituire toglie di mezzo il precedente (metadati e byte), la firma di un altro club non si legge ne si scrive ne si cancella, un PDF e un'immagine da 3 MB vengono rifiutati **anche se Attachment Core li accetterebbe**, cancellare svuota riferimento e allegato, e `readClubSignatureImage` consegna il `data:` URL che il generatore documentale incorpora |
| `tests/ui/club-signature-contract.test.mjs` | 9 | Il contratto della firma sulle superfici: nessun data URL nelle impostazioni, un **solo** modulo che scrive la chiave, nessun componente client che importa il modulo server, nessun `fetch` diretto nel pannello, `PUT` e `DELETE` dietro `canManageClubConfiguration` e `GET` no, i ruoli operativi che non possono scrivere, `access-roles.ts` che **non** ha acquisito un permesso dedicato, e i limiti piu stretti di quelli degli allegati |
...
| `tests/server/payment-reminders.test.mjs` | 23 | **Il sollecito degli insoluti** (W1-F): il residuo dell'anteprima e quello della cassa (130 dovuti, 80 incassati, 50 residuo) e non l'importo dovuto; un atleta senza tutori compare fra i non raggiungibili e non fa fallire il resto; un tutore **senza account ma con email** e raggiungibile; un account che il club non conosce e `no_account`; due invii consecutivi producono **un** messaggio e il secondo dice `already_reminded`; con SMTP non configurato nessuno risulta `sent` e nessuna traccia viene scritta; un invio fallito non ferma gli altri, e ritentabile subito; senza raggiungibili l'azione non parte; una rata di un altro club viene respinta con «Accesso negato» e nessun tutore dell'altro club riceve niente |
| `tests/ui/payment-reminder-contract.test.mjs` | 9 | Il contratto del sollecito sulle superfici: **nessun secondo punto di invio email** fuori da `src/lib/server/email/` in tutto `src/`, nessun canale SMS o push nelle superfici nuove, nessun `fetch` diretto a `/api` dalla finestra, i due elenchi con i quattro motivi, nessuna tabella dentro il dialogo, il pulsante disabilitato durante l'invio, e la rotta dichiarata in entrambi i registri |

...
| Budget previsionale | **PARTIAL — senza interfaccia** | Le collezioni `expected_income` e `expected_expenses` esistono e restano leggibili dal CRUD generico, ma la scheda **Previsti** di `/movements` e sparita con la riscrittura della prima nota (W4-B): un previsto non e un fatto di cassa e non appartiene ad `accounting_entries`, e la sua vecchia scheda si reggeva sull'aggregatore nel browser che la Wave ha rimosso. Nessun dato e stato cancellato. Vedi [16 — Debito tecnico](16-technical-debt.md), W4-B1 |
...
| Report societari | PARTIAL | `/reports`: presenze, gare, categorie e report pagamenti. Il filtro **Periodo** ora tocca anche il report pagamenti — fino alla Wave 4 non lo faceva, e «Ultimo mese» lasciava i quattro numeri finanziari sull'intero storico senza dirlo. Nessun export strutturato: e W4-G |

...
| Verifica email OTP | COMPLETE | Da PP-05 l'indirizzo e **obbligatorio** e si verifica **dopo**: non blocca piu l'accesso. L'avviso «Email non verificata» e la casella del codice stanno sulla pagina Account, percorso completo dal clic alla riga scritta. L'unica limitazione di un indirizzo non verificato e che **non vale come prova di identita** (ADR-0132/0117). Senza SMTP il pulsante fallisce e lo dice, invece di rendere l'account inutilizzabile |
| Verifica telefono OTP | PARTIAL | **Flusso completo e raggiungibile** da PP-05: numero obbligatorio alla registrazione, normalizzato in E.164, OTP monouso con impronta HMAC, scadenza 5 min, 5 tentativi, cooldown 60 s, tre assi di rate limit, una challenge viva per canale garantita dal database. Resta PARTIAL per una ragione sola: **nessun operatore SMS e cablato** — c'e l'astrazione piu il provider `noop` che non spedisce. La scelta dell'operatore e una decisione commerciale aperta (ADR-0131). Con `AUTH_ALLOW_TEST_CODES` il flusso si percorre per intero |
...
| Import atleti | COMPLETE | Wizard in sette passi (file, colonne, verifica, categorie, duplicati, anteprima, import) su un **piano solo** (`src/lib/athletes/import/plan.ts`): ogni riga non vuota ha uno stato e i totali tornano; data di nascita mancante = avviso correggibile; le categorie del file sono **decisioni** (collega a una squadra, crea alla conferma, non importare) e non si creano mai da sole; possibili duplicati (nel file e nel club) decisi dal club, mai fusi. Scrittura sul server (`POST /api/v1/athletes/import`): un atleta per transazione con appartenenza primaria e sede della squadra, idempotente per lotto, audit per scheda e per lotto. Rapporto scaricabile. Forense del file vero in `docs/redesign/IMPORT-ATLETI-FORENSIC.md` ([ADR-0195](18-decision-log.md#adr-0195--limport-di-atleti-e-un-wizard-con-un-piano-solo-nessuna-riga-sparisce-le-categorie-del-file-sono-decisioni-del-club-il-server-scrive-un-atleta-per-transazione-e-riprovare-non-crea-doppioni)) |
...
| Allenatori | COMPLETE | Lista, dettaglio, documenti. Selezione multipla con attiva/sospendi, export PDF **e CSV** per ambito e assegnazione **additiva** a un gruppo operativo (o a una categoria, sui club con una sede sola). **Le squadre mostrate sono della stagione scelta** (ADR-0197 §7): la colonna «Categorie 2026/27», «Nessuna categoria assegnata nella stagione 2026/27» quando non ce ne sono, lo storico sotto con la sua stagione; l'editor offre solo le categorie e i gruppi della stagione e salvare non cancella lo storico. Il riporto di stagione ha il tipo «Assegnazioni allenatori» (spento per scelta, per identificativo, richiede categorie e gruppi; il riepilogo e l'audit dicono quante non hanno trovato una destinazione). Un riferimento per nome vale solo se in tutto il club lo porta una categoria sola; cio che non risolve si conserva com'era. I documenti sono una griglia sola nella scheda (tipo, file, caricamento, scadenza, stato, azioni) e i byte passano da Attachment Core: vedi `src/lib/trainer-documents.ts`. Fino a RC Fix 1 caricare non salvava niente (scriveva su `clubs.trainer_contracts`, colonna inesistente) e «Visualizza» rimandava all'elenco con «Allenatore non trovato» |
...
| Stagioni sportive | COMPLETE | `clubs.settings.seasons`, `/organization?tab=stagioni`. Tre stati (futura, attiva, archiviata) con una sola attiva; filtro server su `x-active-season-id`; creazione guidata con riporto della configurazione dalla stagione scelta (WP-32, WP-35). **La stagione attiva e il perimetro operativo** (ADR-0197): «di quale stagione parliamo?» ha un solo risolutore (`resolveSeasonContext`), lo storico resta con la sua stagione e si scrive nella stagione che si guarda. Il riepilogo del wizard dichiara voce per voce cio che **non** viene riportato. Cambiando stagione la barra laterale e le pagine si aggiornano senza F5 |
| Eliminazione di una stagione | COMPLETE | Azione «Elimina» nella scheda Stagioni (ADR-0197 §9): cassetto con l'impatto (`GET /api/v1/seasons/:id`: configurazione che si cancella, riferimenti che si staccano, storia che blocca), conferma scritta «ELIMINA <nome>» verificata dal server, `DELETE /api/v1/seasons/:id`, permesso `seasons.delete`. La stagione attiva non si elimina finche un'altra non e attiva; una stagione con eventi, presenze, movimenti, tesserati, rate, pratiche o appuntamenti si **blocca** con l'elenco: si archivia. Audit `season.delete.requested` (anche sul rifiuto) e `season.deleted` |
| Riporto fra stagioni | COMPLETE | `/api/v1/seasons/:id/rollover`. Copia categorie, sconti, piani, gruppi numerazione, programma settimanale e previsionale; idempotente, rimappa i riferimenti, non tocca i dati operativi. **I tesserati entrano solo se scelti** (ADR-0196): con «Tesserati nelle squadre» fra i tipi `athleteIds` e obbligatorio, `[]` porta zero, `null`/assente si rifiuta; il wizard propone l'elenco senza nessuno spuntato e l'audit dichiara proposti/riconfermati/creati |
...
| Generazione automatica allenamenti | COMPLETE | `src/lib/server/training-automation.ts` + `POST /api/v1/training-automation` (un club, a mano) e `GET` (tutti i club, da cron). **Genera nella stagione dichiarata** (o attiva, dal cron) e risponde con la diagnostica del programma: quante voci, quante generabili, quante no e perche (altra stagione, disattivate, categoria non nel catalogo **della stagione**, ambigua), occorrenze fuori dal periodo della stagione; il pannello la mostra per esteso al posto di «non contiene sessioni valide» (ADR-0197 §4). Le voci del programma di altre stagioni **non spariscono** salvando: la colonna riscritta conserva le altre stagioni (§5). Dal 2026-08-28 `vercel.json` lo schedula alle **04:00**: prima esisteva ma non girava. Si autentica con `CRON_SECRET`; un club che fallisce non ferma gli altri |
| «Genera fino a...» e anteprima | COMPLETE | Stesso endpoint (`untilDate`, `preview`), stesso planner e stesso scrittore canonico — nessun secondo generatore (WP-03, WP-17). L'anteprima non scrive niente e mostra creati/gia esistenti/conflitti/esclusi prima di confermare; l'esecuzione mostra lo stesso riepilogo dopo. Raggiungibile dal pannello "Assistente Automazione" in `/training`. Limite: 366 giorni da oggi (`MAX_MANUAL_GENERATION_DAYS_AHEAD`) |
| Impatto di una modifica al programma settimanale sugli eventi futuri | COMPLETE | `POST /api/v1/training-automation/schedule-impact` (WP-08). Dopo l'autosave del programma, un avviso non bloccante in `WeeklyTrainingSchedulePanel` dice quanti allenamenti futuri gia generati la modifica riguarda e quanti si possono aggiornare in sicurezza (nessuno annullato, con presenze o gia modificato a mano — [ADR-0171](18-decision-log.md#adr-0171--un-evento-generato-sa-dire-se-e-ancora-quello-che-lautomazione-ha-scritto)); due scelte esplicite, "solo alle nuove generazioni" o "aggiorna anche i futuri non modificati". Una voce **rimossa** non tocca mai niente (WP-14) |
| Sospensioni/eccezioni alla generazione | PARTIAL | `settings.trainingAutomation.exclusions` (WP-15): un salto singolo o un intervallo, club-wide o per singolo slot (`slotId`). Raggiungibile dal pannello "Assistente Automazione" **solo per il caso club-wide**; il selettore per limitare una sospensione a un singolo slot non ha ancora una UI (il modello e il backend lo sostengono, [ADR-0175](18-decision-log.md#adr-0175--sospensioni-ed-eccezioni-un-salto-e-una-sospensione-sono-la-stessa-cosa-con-un-intervallo-diverso)) |
...
| Convocazioni | COMPLETE | `MatchConvocations`, sopra `club_event_participants.convocation_status` (ADR-0099). Dal 2026-09-07 anche **in lettura e dalla pagina del club**: la rosa si contava su dieci grafie del payload della gara che nessuno scrive piu — la scheda diceva «0/16» dopo aver convocato undici atleti, e riaprendo la finestra la rosa risultava vuota, quindi salvare una seconda volta la cancellava. E dalla pagina Gare la convocazione **non si salvava affatto**: scriveva `clubs.matches`, che il server rifiuta. Il calendario porta ora `convocated_count` per evento, le due schermate rileggono le righe con `listEventParticipants` e la pagina Gare passa da `saveEventConvocations` |
...
| Lettura documenti d'identita | PARTIAL | **Non piu sulla scheda atleta** (PP-01 §H, vedi [42](42-pp-01-club-atleti-allenamenti.md)): li la funzione e stata tolta dalla superficie perche l'esperienza — fotocamera a tutta pagina, dialogo largo, OCR sull'immagine di una scrivania — e stata giudicata non utilizzabile, e verra riproposta con un percorso vero. Il motore **non** e stato toccato: la scheda ne duplicava una copia in linea, e le altre schermate montano il campo condiviso. Flusso unico su nuovo atleta, allenatore, staff, socio e genitore/tutore: si carica, si vede cosa e stato letto, si sceglie cosa applicare. Motore: OCR locale (`tesseract.js`), il documento non lascia il browser. **Legge**: JPG, PNG, WEBP, HEIC fino a 8 MB, piu la zona a lettura ottica (MRZ) quando c'e. **Non legge**: PDF — rifiutati con una spiegazione, non accettati per poi fallire (serve `pdfjs-dist` per rasterizzare: e una decisione di dipendenza, non un difetto). 22 test |
...
| Moduli online pubblici | COMPLETE | `/forms/[publicSlug]` con slug a suffisso casuale, `GET|POST /api/public/forms/[publicSlug]`, rate limiting per IP, formati ristretti, priorita smartphone. Il vincolo «si compila una volta sola» **non vale su questo canale** e la casella nel builder lo dice: qui chi compila non e ancora in archivio, e non c'e nessuno da riconoscere |
| Moduli online nel fascicolo della famiglia | COMPLETE | **PP-02 §G/§J**. La card «Moduli online» elenca cio che il club ha pubblicato con lo stato di **questo figlio**, e la CTA porta a compilarlo dentro l'area famiglia, mai fuori. Una pratica ha **tre** tipi, non due: `enrollment`, `renewal` e `submission` — un questionario di gradimento non e un rinnovo, e arrivava in segreteria etichettato come tale, da approvare. Il tipo si **deriva** dal modulo (`isEnrollmentForm`), e da lui dipendono anche le parole della schermata e se la pratica porta una stagione. Un modulo con il termine scaduto non offre la CTA |
| Pratica di iscrizione: bozza, integrazione, revisioni, dichiarazioni, riconoscimento delle prove | COMPLETE | ADR-0189/0191/0192/0193 (2026-09-16). «Salva e continua dopo» sul server (gettone 256 bit, 30 giorni, consumato all'invio); stati `pending · changes_requested · approved · converted · rejected · archived`; «Richiedi integrazione» con i campi da correggere ed email; reinvio dalla ricevuta (`/iscrizione/:reference/integra`) limitato a quei campi, con la copia precedente in `form_submission_revisions`; dichiarazioni con testo mostrato e impronta; `snapshot_hash`; in revisione le persone in prova che somigliano (`trialCandidates`) e «Approva e converti» via ADR-0188; coda con contatori per stato. Il pubblico non ha nessuna rotta che cerchi persone. Aperti: allegati in bozza, OTP sul recapito, retention automatica delle pratiche chiuse |
| Builder a blocchi ed editor ProseMirror | COMPLETE | ADR-0190 (2026-09-16). Tavolozza Contenuto · Campi · Dichiarazioni · Evidenza grafica; blocco `content` (HTML sanificato **sul server**: `src/lib/rich-text/sanitize.ts`); `legalKind` sulle caselle (presa visione, accettazione richiesta, consenso facoltativo mai obbligatorio, autorizzazione); «Mostra solo se» (una condizione, valutata anche dal server); tipi e dimensione dei caricamenti; `image_upload`; anteprima web `/modulistica/moduli/:id/anteprima` desktop e 375 px. Editor unico (`RichTextEditor`, TipTap): titoli, dimensione, allineamento, interlinea, spazio, elenchi, link, tabelle, immagini negli allegati con larghezza/allineamento, interruzione di pagina come nodo, segnaposto inline atomici, annulla/ripeti. `DocumentEditor` e un involucro di quell'editor. Aperto: PDF server (D38), FEA/FEQ (fornitore), conservazione a norma |
...
| Ciclo di vita di un bando | COMPLETE | `draft → active → closed`, con la riapertura. Prima i tre valori esistevano e non governavano niente: ogni programma nasceva bozza, nessuna schermata sapeva cambiarlo, e una bozza iscriveva e maturava come un programma attivo. Adesso una bozza **non iscrive e non matura**, lo stato si cambia solo da `POST /api/v1/funding/programs/:id/transition`, e l'interfaccia offre le transizioni che il dominio ammette (N6) |
| Tutti i periodi di un bando, non solo i calcolati | COMPLETE | La scheda mostrava le sole righe di maturato, e il ricalcolo si ferma a oggi: i mesi futuri non comparivano. I periodi si **derivano** dalla configurazione (ADR-0037 §4) e si fondono con le righe; un periodo senza riga esce come «previsto», che non e «non maturato» — il primo dice che nessuno ha guardato, il secondo che l'atleta non ha frequentato abbastanza (N8) |
...
| Compensazione contributo → rata della famiglia | COMPLETE | **La scelta di ADR-0037 resta**: quale parte della quota il voucher copre lo decide il club, non l'importo maturato, e compensare in automatico farebbe risultare saldate rate che nessuno ha pagato. Cio che arriva e la **decisione esplicita**: `payment_coverage_allocations` collega una rata a un'adesione per un importo, e una rata puo averne piu di una (600 di piano = 500 di voucher + 100 di famiglia). L'allocazione e una **promessa**: non scrive nessun `payment_transaction`, non tocca `payments.status`, non entra in prima nota. Sei grandezze separate — debito, copertura prevista, maturata, liquidata, quota famiglia, incassi reali — e le prime quattro parlano dell'ente, le ultime due della famiglia. Due tetti: la copertura viva di una rata non supera la rata, quella di un'adesione non supera l'assegnato. Non si cancella, si storna ([ADR-0158](18-decision-log.md#adr-0158--un-voucher-copre-una-rata-non-la-paga-lallocazione-e-una-promessa-e-la-cassa-resta-cassa)) |
...
| SMS | PARTIAL | Astrazione di trasporto (`src/lib/server/sms/`), unico punto di invio `sendSms`, provider `noop` che non spedisce. **Nessun operatore reale cablato**: decisione commerciale aperta (ADR-0131). Nessun SMS applicativo oltre all'OTP |
...
| Test mobile | PARTIAL | `easygamemobile/tests/`, 118 test (`node --test`) sui moduli puri di Identity & Access, sui permessi/documenti/categorie Trainer di WP3, e sull'intero dominio Parent WP4-9 (multi-figlio, calendario/RSVP, notifiche, pagamenti/consensi/documenti, appuntamenti/strutture/orari, profilo atleta); nessun test sulle schermate (nessun renderer RN installato) o sul layer dati/rete |
...
| **Registrare il bonifico dell'ente su un periodo** | **Funziona** (N15, ADR-0160). «Registra liquidazione» sta sulla riga del periodo maturato, accetta accrediti **parziali**, e la riga scritta **e** il movimento bancario del club: stesso conto, verso in entrata, causale congelata, e nell'estratto conto si legge «Incasso voucher <bando> — <atleta> — <periodo>». Non nasce nessun pagamento della famiglia |
| Stornare un accredito | **Funziona.** Movimento inverso, storico preservato, e il periodo torna fra i crediti verso l'ente. Agisce sulla testata: su un bonifico in blocco la schermata lo dichiara prima |
| Vedere quanto l'ente deve ancora versare | **Funziona.** «Voucher da ricevere» = maturato meno liquidato, per periodo e per adesione. Non «previsto meno liquidato»: cio che non e maturato non e ancora un credito |
...
| Capability | Stato | Note |
|-----------|-------|------|
| Calendario unico | COMPLETE | `/calendar`: allenamenti e gare insieme, filtrabili per tipo, sede, categoria, gruppo e intervallo. **Della stagione che il browser mostra**, per identita e non per data (ADR-0197 §6): due stagioni sovrapposte non si mescolano, `season_id=` sceglie una stagione precisa, `all_seasons=1` le mostra tutte per chi lo chiede; un evento nuovo nasce nella stagione mostrata e una modifica non lo sposta. Vale per `/training` e `/matches`, che mostrano la stagione nell'intestazione. Prima non esisteva niente da unire, perche non esisteva un'entita comune da elencare (ADR-0098) |
...
| Capability | Stato | Note |
|-----------|-------|------|
| Selettore figli | COMPLETE | Anche in club diversi; il legame sopravvive al ricaricamento (5A). **PP-02 §A**: la scelta si fa in una schermata sua — nome, foto, anno, tutte le categorie con la sede, e lo stato di chi non e piu iscritto — e si cambia dalla barra laterale e dal menu mobile. La fascia «Stai vedendo …» sopra il contenuto di tredici pagine su tredici **non c'e piu**: era l'informazione giusta nel posto sbagliato, e a 375 px spingeva sotto la piega cio per cui la pagina era stata aperta |
| Il legame vale senza tessera di club | COMPLETE | **PP-02 §A** ([ADR-0127](18-decision-log.md#adr-0127--il-legame-di-un-tutore-non-e-la-sua-tessera-il-suo-indirizzo-non-e-un-legame-che-apre-da-solo)). Un tutore collegato ma senza riga in `organization_users` non trovava **nessun** figlio: il vaglio del legame girava su un insieme di candidati scelto dalle tessere. L'indirizzo di contatto resta invece un legame **solo dove il tutore e gia entrato**: lo scrive la segreteria a mano, e un refuso e l'indirizzo verificato di un'altra persona |
| Un identificativo sconosciuto non apre il figlio sbagliato | COMPLETE | **PP-02 §A**. `getParentDashboardData` ricadeva su `linkedAthletes[0]` per ogni identificativo non-UUID: un segnalibro storto mostrava importi e stato del certificato **dell'altro figlio**, con il nome giusto accanto. Adesso risponde `null`, e il guscio porta alla schermata di scelta |
...
| «Paga ora» | COMPLETE | Riusa il checkout esistente con l'identita di sessione. Nessun secondo checkout. **PP-02 §D**: il **motivo** per cui a volte non si puo pagare si conosce **prima** del clic. Con la societa che non ha configurato gli incassi online il pulsante restava acceso e il motivo arrivava come errore rosso; senza rate aperte si spegneva con il motivo dentro un `title` del browser, che su un telefono non esiste. Adesso il canale arriva dal server (`payments.online`) e la frase sta accanto al pulsante, sulla pagina **e** su ogni riga del piano. I sei ostacoli del dominio diventano due fatti utili a una famiglia — «si salda in segreteria» oppure «riprova piu tardi» — e nessun messaggio nomina l'abbonamento o il conto della societa |
| Ricevute e documenti di pagamento | COMPLETE | Il gate e il legame. **PP-02 §E** ([ADR-0128](18-decision-log.md#adr-0128--cio-che-la-famiglia-legge-di-un-documento-di-pagamento-e-un-elenco-chiuso)): un elenco solo per ricevute e fatture, con data, numero, causale, importo, **figlio**, stato, «Visualizza» e «Scarica». E soprattutto una **proiezione chiusa**: prima usciva la riga intera — chi ha emesso, chi ha annullato, la classificazione contabile congelata e le chiavi di riconciliazione della cassa del club. Una ricevuta annullata si legge «Annullata» e non sparisce; il motivo dell'annullamento resta del club |
...
| Appuntamenti su slot | COMPLETE | Dal dominio 5E. **PP-02 §K**: il club puo dire **se** riceve richieste online — esisteva solo `active` sulla singola fascia, e chi voleva chiuderle doveva spegnerle a una a una — e **per cosa**: un elenco di motivi, ognuno prenotabile o riservato al desk. Un motivo e **un nome e un interruttore**: durata, sede e operatore li dice la fascia, che e dove sono gia onorati, e mostrarli sul motivo faceva credere che decidessero qualcosa. La famiglia sceglie fra quelli invece di scrivere un testo libero, e con le richieste chiuse il modulo non compare affatto. Un club che non ha configurato niente continua a ricevere (W6-D03); uno che ha configurato dei motivi e li ha lasciati **tutti** al desk non riceve piu, e la schermata lo dice — prima diceva il contrario e ogni invio veniva rifiutato |
| Moduli online nel fascicolo della famiglia | COMPLETE | **PP-02 §G**. L'area «Moduli online» era un **rimando** alla pagina Iscrizione: il posto e giusto — ospitare il motore due volte sarebbe la seconda implementazione di un dominio che ne ha gia una — ma non rispondeva alla domanda per cui la card esiste, «cosa devo ancora fare». Adesso e un elenco con titolo, figlio, stato (Da compilare / Inviato / Completato / Scaduto), scadenza e data di invio, letto da `GET /api/v1/family/online-forms`. «In compilazione» non c'e per costruzione: una bozza vive nel browser che l'ha cominciata |
| Modulo che si compila una volta sola | COMPLETE | **PP-02 §J**. La sola difesa era la deduplicazione a finestra — dieci minuti, stesse risposte — che protegge dal doppio clic e non dalla compilazione ripetuta: fuori da quella finestra la stessa iscrizione si rimandava tre volte. `settings.singleSubmission` sta **dentro la versione pubblicata**, il presidio gira prima di caricare gli allegati, una pratica **respinta non blocca**, e senza un soggetto risolvibile non si vincola niente — un conteggio per indirizzo email bloccherebbe due fratelli iscritti dallo stesso genitore |
| Anteprima di un modello del catalogo | COMPLETE | **PP-02 §J**. La scheda diceva titolo, classe e provenienza e non una parola su **cosa chiede** il modulo: per saperlo bisognava adottarlo, aprirlo e cancellarlo. Adesso ogni voce apre l'elenco dei propri campi, e il pulsante dice cosa succede — «Usa modello», non «Adotta» |
...
| Cancellazione di un atleta | COMPLETE | Dialogo dell'applicazione, con le conseguenze scritte. Le tre azioni irreversibili della scheda passano da un meccanismo solo. Il testo prometteva «la scheda, le appartenenze e i certificati medici collegati vengono rimossi»: da quando `assertPersonalDataDisposed` presidia `deleteResource` quella frase era **falsa per quasi ogni atleta reale** — l'iscrizione online crea richieste documentali, i moduli registrano consensi — e adesso dice cosa succede davvero e dove sta l'altra strada. Il motivo del server non viene piu sostituito da «Errore nell'eliminazione dell'atleta»: la guardia elenca **quali** dati restano, e quell'elenco arriva a chi legge |
| Eliminazione in blocco di atleti | COMPLETE | Il ciclo non aveva un `try` per singolo elemento: al primo atleta con dati personali vivi l'errore usciva dal `for`, i successivi non venivano **nemmeno tentati**, e restava una cancellazione parziale che nessuno dichiarava. Adesso ogni elemento e tentato per conto suo e l'esito e detto per intero — quanti eliminati, quanti no e perche — e la conferma avverte **prima** che l'operazione puo riuscire a meta |
| Lettura di un documento (OCR) | COMPLETE | Locale, nel browser. Accetta ora anche il **PDF che contiene una fotografia** — cioe cio che salva un telefono che «scansiona». Fotocamera in **quattro** schermate piu il dialogo del tutore: PP-01 §H ha tolto la quinta, lo scanner proprio della scheda atleta, e con essa la copia in linea del motore. **Si propone, non si scrive** |
...
| Cambio di categoria consapevole, sede derivata dalla squadra, storia intatta | COMPLETE | [ADR-0194](18-decision-log.md) (2026-09-16). Un editor solo (`AthleteCategoryMembershipEditor`: scheda e creazione) che sceglie **squadre** («Pulcini · S. Cosma») e chiede cosa fare della primaria precedente (default: rimuovila); «Cambia categoria» in blocco con ruolo, due politiche visibili, **anteprima del server** e conferma; writer unico e transazionale con audit prima/dopo per atleta e per blocco; coppia categoria/sede vagliata sul server (anche sul registro generico); prova, iscrizione online e import parlano di squadre; export con la sede della riga. Presenze, convocazioni, eventi e audit non cambiano al cambio di categoria: il rapporto per categoria tiene chi ha una riga («oggi in altra categoria»), l'appello fotografa `is_extra_category` alla prima registrazione. Aperti: validita temporale delle appartenenze (D-RD-25), pulizia del dato UAT del pilota (D-RD-26) |
...
| Certificato medico | COMPLETE | Quattro stati con **«in scadenza»**, la data del certificato che governa — non del piu vecchio — in tutti gli stati, e una CTA per portarne uno nuovo. L'upload manda finalmente il **tipo**: senza, un certificato entrava come «altro» e non muoveva lo stato sanitario. **PP-02 §F**: stato e data sono **una riga sola** composta dal dominio — `Valido — Scade il 01/06/2027`, `In scadenza — Scade il 20/09/2026`, `Scaduto — Scaduto il 03/01/2026`, e `Data di scadenza non disponibile` quando la data non c'e. Compare uno stato che mancava, `undated`: un certificato **consegnato senza scadenza** non e un certificato mancante, e finiva su «Certificato mancante» perche l'unico dato guardato era la data. E la resa della data dichiara il fuso: prima la faceva la schermata con quello del lettore, e in un fuso positivo un certificato che scade il primo giugno si leggeva «31/05» (stessa famiglia di AUD-02) |
...
| Accesso EasyGame di un atleta | COMPLETE | Invito, riscatto, revoca e audit, con i **quattro** stati sul pannello del club — nessun account, invito inviato, account attivo, accesso revocato — derivati dall'ultimo invito senza nessuna colonna nuova (ADR-0115/0121). **Mai una password in chiaro**: un link opaco, poi la persona sceglie. La revoca vale sull'**identita**: toglie le tessere il cui ruolo **risolto** e `athlete`, alias e ruoli personalizzati compresi (ADR-0114). **L'indirizzo dell'invito non puo essere quello di un tutore della stessa scheda** (ADR-0124): un accesso che vive nella casella del tutore non e l'accesso dell'atleta, e il rifiuto e un 400 che dice cosa fare. **E un'utenza e l'accesso di una scheda sola** (ADR-0125): un indirizzo con un invito vivo su un'altra scheda e rifiutato con 400, e il riscatto rifiuta comunque, dentro la transazione che scrive. «Scollega account» chiude adesso anche bacheca e RSVP, non solo l'area atleta |
| Athlete Dashboard V1 | COMPLETE | **Tredici** pagine, tutte nel menu e tutte raggiungibili: home, squadre, calendario, allenamenti, convocazioni, gare, presenze, storico, bacheca, notifiche, documenti, appuntamenti, profilo. Proiezione a elenco chiuso (`CAMPI_AREA_ATLETA`): fuori per scelta denaro, tutori, altri atleti e contenuto clinico. Verificata a schermo a 375 / 768 / 1280 / 1440 (PP-04) |
| «Le mie squadre», «Allenamenti», «Storico» | COMPLETE | Le tre pagine che l'area prometteva dal menu e non aveva. Lo storico dichiara il proprio limite — la sola stagione attiva — invece di lasciarlo credere (PP-04) |
| Multi-categoria dal lato dell'atleta | COMPLETE | Di un evento congiunto escono **solo** le categorie dell'atleta, incrociate lato server (ADR-0120). Il nome della squadra si risolve dal catalogo del club: `athlete_category_memberships.category_name` e nullable, e la pagina stampava due UUID |
| Recapiti modificabili dall'atleta | COMPLETE | Sei campi, elenco **chiuso** lato server: un campo nuovo su `athletes.data` nasce non scrivibile da quella porta. Nome, nascita, codice fiscale, numero e categoria restano della societa |
| Accesso di un minore a nome proprio | PARTIAL | Il codice **non decide** la policy: pretende che una persona dichiari la responsabilita genitoriale, e la dichiarazione resta nell'audit (ADR-0116). Le domande legali aperte sono **cinque** — la quarta e se un minore possa avere un accesso che vive nella casella del tutore, a cui ADR-0124 risponde **no** in via conservativa — e sono nel verbale [45](45-pp-04-athlete.md): **richiedono validazione umana**. Il flusso che il verbale chiamava normale, invitare il minore sulla casella di famiglia, e ora **rifiutato**: creava il secondo cappello dell'account del genitore, non un accesso del minore. La quinta domanda (ADR-0125): due fratelli minorenni **non** possono avere la stessa casella come accesso atleta, perche un indirizzo e un'identita |
| Autogestione dell'atleta maggiorenne | MISSING | Quote e consensi restano di chi ha la responsabilita: il cruscotto della famiglia lo apre un tutore (ADR-0118/0122). Per un atleta adulto serve una superficie **sua**, con la sua proiezione — debito PP04-D5 |
| Policy di club su cosa l'atleta vede | MISSING | `clubs.settings` non porta nessuna chiave a tema e non c'e nessuna schermata da cui accenderla. Nel frattempo vale l'invariante presidiata: l'atleta vede **solo le proprie** presenze — debito PP04-D3 |
...
| Export e cancellazione di una persona | COMPLETE | Attraversa sei indici polimorfi. Per un minore serve un riepilogo confermato. **Fino al rimedio era COMPLETE senza una porta**: il dominio e le tre rotte sotto `/api/v1/data-subject` esistevano, e in `src/app` e `src/components` avevano **zero chiamanti** — mentre `assertPersonalDataDisposed` rimandava proprio li («usa la cancellazione dei dati personali»), rendendo incancellabile quasi ogni atleta reale. La superficie e `src/components/athletes/profile/athlete-data-subject-section.tsx`, montata sulla scheda atleta sotto «Accesso EasyGame»: mostra l'**inventario** prima di chiedere conferma — cosa sparisce, cosa resta senza nominare nessuno, cosa la societa e tenuta a conservare **con il motivo** — manda il gettone che il dominio pretende (e quando il server dice che il riepilogo e cambiato ricarica invece di rigenerarlo di nascosto), pretende la spunta esplicita per un minore, e conferma con `AlertDialog`, non con `window.confirm`. Visibile solo a chi ha `data_subject.export` / `data_subject.erase`, lette **separatamente**: chi ha solo l'export vede mezzo pannello. L'export dichiara `clinicalContentOmitted` a chi non ha `clinical.read` |

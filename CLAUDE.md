...
| Data di rilascio/scadenza di un tesseramento | `src/lib/server/resources.ts` (`assertRegistrationDates`) | Stesso punto di `assertRegistrationFederations`: l'unica strada per `athletes.data`. Una scadenza senza rilascio, o prima del rilascio, si rifiuta — **solo per le righe nuove o cambiate**: una riga storica gia invalida non si tocca (ADR-0199 Wave E, §E1-E4) |
| Import di un `.docx` per la Modulistica | `src/lib/server/docx-import.ts` | Converte e sanifica, **non salva**: il salvataggio riusa `POST /api/v1/document_templates`. `mammoth` non apre mai `vbaProject.bin` (nessuna macro); le immagini incassate non diventano `data:` URI (dichiarate in `unsupported`, mai perse in silenzio); la dimensione non compressa dell'archivio si vaglia dai metadati dello zip prima di decomprimere niente (ADR-0199 Wave E, §E5-E12) |
